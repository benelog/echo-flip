...
// 저장소 키 접두사가 바뀐 책(예: 앱 이름 변경)을 위한 마이그레이션.
// 옛 접두사의 값을 한 번만 새 접두사 키로 옮겨 독자의 북마크·형광펜을 보존한다.
const MIGRATED_NAMES = ['bookmarks', 'highlights', 'toc-hidden']

export function migrateLegacyKeys(legacyPrefixes, prefix) {
  try {
    for (const legacy of legacyPrefixes) {
      for (const name of MIGRATED_NAMES) {
        const oldKey = `${legacy}-${name}`
