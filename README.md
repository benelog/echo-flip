...
- 양방향 카드(A면: 영어·용어 / B면: 뜻·설명) + 덱 관리, 태그
- 학습: 방향 선택(A면→B면 / B면→A면) → 카드 뒤집기 → 스스로 맞음/틀림 판정 → 덱 소화 후 틀린 카드 재도전 라운드
...
- CSV 가져오기/내보내기 (`side_a,side_b,type,tags,phonetic,example`, 태그는 `|` 구분; 가져오기는 구 `front,back` 헤더도 인식)
