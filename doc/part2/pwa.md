...
직접 쓴 일흔 줄 남짓한 파일이고, Workbox 같은 라이브러리를 쓰지 않았다.
...
::: info [용어 풀이] Cache Storage(캐시 저장소)
...
"어제의 사본을 써도 화면이 틀려지지 않는다"는 말에는 조건이 하나 붙는다.
...
그래서 서버가 no-store를 붙인 개인 화면이, 우리가 직접 쓴 `sw.js` 코드에 의해 사용자 기기의 Cache Storage에 그대로 저장된다.
...
같은 파일 하나가 이 세 층에 각각 다른 나이의 사본으로 존재할 수 있다는 것이 캐시를 다루기 어렵게 만든다.
...
그래서 오프라인에서도 앱의 껍데기는 즉시 뜨고, 데이터를 IndexedDB에 미리 내려 두면 화면까지 그릴 수 있다.
...
서비스 워커는 코드가 일흔 줄이어도, 실수의 결과가 사용자 브라우저에 캐시로 남는 층이다.
...
셋 다 잊었을 때 사용자 기기에 흔적이 남는 종류의 실수라 지침 가치가 크다.
...
페이지는 network-first라 늘 최신인데 CSS만 캐시의 옛 사본이 나오는 조합을 막기 위한 것이고, 지문이 맞는 주소는 1년짜리 `immutable`로 캐시할 수 있게 되는 이득도 따라온다.
