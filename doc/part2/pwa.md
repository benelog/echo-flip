...
Echo Flip은 앞의 두 조건을 `manifest.webmanifest`로, 마지막 조건을 `sw.js`로 충족한다.
두 파일 모두 `internal/web/static/`에 있는 평범한 정적 파일이고, 9장에서 본 embed로 Go 바이너리에 담겨 다른 정적 자산과 같은 경로로 서빙된다.
...
그런데 이 앱에서는 걸러 내야 할 "데이터"가 `/api/`로 끝나지 않는다.
9장에서 봤듯 페이지 HTML은 서버가 요청 시점에 렌더링하고, 홈 화면의 복습 큐도 통계 화면의 숫자도 HTML 안에 이미 들어 있다.
페이지가 곧 데이터다.
...
낡은(stale) 사본으로 먼저 그리고 뒤에서 재검증(revalidate)한다고 해서 stale-while-revalidate라 부르는 전략이다.
...
서비스 워커는 저절로 켜지지 않는다.
10장에서 해부한 `app.js`, 곧 이 앱에 남은 유일한 자바스크립트 파일의 한 대목이 등록을 맡는다.
...
10장에서 "그래도 남는 자바스크립트"의 목록에 서비스 워커 등록이 들어 있던 이유가 이것이다.

조건은 `"serviceWorker" in navigator` 하나로, 지원하지 않는 브라우저를 걸러 낸다.
개발 환경과 운영 환경을 가르는 분기는 없다.
...
그래도 CSS 같은 정적 자원은 stale-while-revalidate 탓에 새로고침 한 번 늦게 보일 수 있는데, 개발자 도구 Application 탭의 "Update on reload"를 켜 두면 이 어긋남도 사라진다.
...
`const CACHE = "echo-flip-v2"` 한 줄에 이 구조의 약점이 숨어 있다.
...
stale-while-revalidate가 뒤에서 갱신해 주므로 새 파일은 그다음 방문에 반영되고, 16장에서 본 한 시간짜리 `Cache-Control`까지 겹치면 스타일 변경이 최대 한 시간과 방문 한 번만큼 늦게 보일 수 있다.
페이지 HTML은 network-first라 내용이 늦는 일은 없고, 껍데기의 단장이 잠시 늦는 것은 감수하기로 한 비용이다.

그보다 무거운 순간은 캐시 전략 자체를 바꿀 때다.
`sw.js`를 고쳤다면 `CACHE` 값을 올려야 `activate` 훅이 옛 캐시를 통째로 지우고 새 정책으로 다시 시작한다.
이 파일의 캐시 이름이 v1이 아니라 v2인 것이 바로 그 손잡이를 한 번 당긴 흔적이다.
