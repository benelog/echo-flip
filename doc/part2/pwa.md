# 20장 PWA: 설치되는 앱으로 만들기

17장에서 앱을 Vercel에 올렸고, 18장과 19장에서 인증과 데이터베이스를 붙였다.
...
첫째, HTTPS로 서빙되어야 한다.
17장에서 봤듯 Vercel이 인증서를 자동으로 발급하고 갱신하므로 이 조건은 저절로 충족된다.
...
21장에서 볼 스케줄 워크플로로 흉내 낼 수는 있지만, 사용자별 시간대와 알림 시각을 관리하는 순간 이 앱은 다른 앱이 된다.
...
다음 21장에서는 이 모든 것을 떠받치는 무료 티어의 한도를 정면으로 들여다본다.
