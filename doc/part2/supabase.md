# 15장 Supabase: 인증과 데이터베이스

14장에서 정적 프런트엔드와 Go 서버리스 함수를 Vercel 한 곳에 배포하는 과정을 살펴봤다.
...
로그인 상태는 여러 화면이 함께 쓰므로 9장에서 다룬 Context로 공급한다.
...
패키지 전역 변수 `jwks`를 `sync.Once`로 한 번만 초기화하는 것은 14장에서 본 서버리스 함수의 생명주기 때문이다.
...
이 값은 Supabase `auth.users` 테이블의 UUID이고, 11장에서 설계한 `profiles.id`가 이 값을 그대로 쓴다.
...
그런데 11장에서 다룬 스키마 마이그레이션은 반대로 풀러를 거치면 안 된다.
...
14장에서 본 `vercel.json`은 함수 리전을 고정하고 있다.
...
11장에서 설계한 RLS 전략이 이 장의 인증 구조와 어떻게 맞물리는지만 짧게 복기한다.
