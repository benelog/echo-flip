...
`card_type`은 5장에서 `check` 제약으로 흉내 냈던 값 목록이 enum 타입이 된 모습으로, 타입 자체가 문서가 되고 저장 공간도 작지만 값 목록 변경이 DDL이고 특히 값 제거는 불가능에 가깝다는 대가가 있다.
...
이 두 프로젝트는 14장에서 정한 브랜치 정책, 16장에서 설정한 배포 구분과 한 축으로 정렬된다.
