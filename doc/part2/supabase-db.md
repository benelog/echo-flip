...
4장에서 본 B-tree 인덱스는 한 칸에 값 하나가 든 경우를 정렬해 찾는 구조라 배열 안의 개별 원소는 훑지 못한다.
...
아래 블록을 읽기 전에 연결 문자열의 구조부터 짚어 두자.
연결 문자열은 `방식://사용자:비밀번호@주소:포트/DB이름` 형식이고, 이 장에서 계속 나온 6543과 5432가 끝에서 두 번째 자리의 포트 번호다.
이 앱에서 두 연결 문자열이 서로 다른 곳은 그 숫자뿐이다.
...
"서버는 트랜잭션 풀러(6543), 마이그레이션은 직접 연결(5432)을 쓴다", "마이그레이션은 개발 프로젝트에 먼저 적용한다", "운영 연결 문자열로는 조회 외의 문장을 실행하지 않는다."
