...
```bash
# 로컬 모드(1부)는 이 파일이 필요 없다. Supabase에 연결할 때만 셸에서 export
# 하거나 direnv로 불러온다 — Vercel 환경변수에도 동일하게 등록.
...
# 웹 로그인(OAuth)과 토큰 검증에 필요. Supabase → Settings → API
SUPABASE_URL=https://<project-ref>.supabase.co
SUPABASE_ANON_KEY=<anon key>
# JWKS URL은 SUPABASE_URL에서 자동 유도된다. 직접 지정하려면:
