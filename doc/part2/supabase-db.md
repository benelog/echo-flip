...
17장에서 봤듯 서버리스 함수의 파일시스템은 인스턴스가 회수되면 함께 사라지는 일회용 공간이라, 거기에 쓴 SQLite 파일은 언제든 증발한다.
...
18장에서 본 무상태 인증과 이 결정이 맞물려, 검증할 표면적이 Go API 한 곳으로 줄어든다.
...
포트가 6543과 5432로 갈린 것은 이 장에서 본 풀러와 직접 연결의 차이다.
