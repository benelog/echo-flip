...
마지막으로 함수와 데이터베이스를 같은 리전에 두는 콜로케이션, 브라우저의 직접 접근 통로를 잠그는 RLS 전략, 개발용과 운영용 데이터베이스의 분리와 그 스키마를 브랜치가 정한 DB에 자동으로 적용하는 구성, 이 모든 연결을 묶는 환경 변수 구성을 정리한다.
...
::: info [용어 풀이] 연결 문자열(Connection String)
...
::: info [용어 풀이] GIN 인덱스(Generalized Inverted Index)
...
우리 서버의 풀(pgxpool)이 DB에 직접 붙는 대신 풀러에 붙고, 풀러가 소수의 실제 Postgres 커넥션을 여러 클라이언트에게 나눠 쓰게 한다.
...
::: info [용어 풀이] 권고 잠금(Advisory Lock)
...
자동으로 복구를 시도하지 않고 멈춰 서는 이 태도는, 되돌리기 어려운 대상을 다루는 도구가 갖춰야 할 기본자세다.
...
이미 쓰는 두 플랫폼 안에서 Vercel의 Preview 스코프와 Supabase의 두 번째 무료 프로젝트로 꾸리는 지금 구성이, 추가 플랫폼 없이 운영과 같은 스택을 유지하는 답이었다.
...
자동 적용을 하려면 5432 문자열을 함수의 환경 변수에도 넣어야 하는데, 그것은 스키마를 바꿀 수 있는 자격을 늘 켜져 있는 함수에 상시로 쥐여 주는 일이다.
...
::: info [용어 풀이] 확장 후 축소(Expand and Contract)
...
같은 목록을 개발 프로젝트 값으로 채워 `.env.dev`라는 이름으로 저장해 두면 앞 절의 `run_dev.sh`와 `migrate_dev.sh`가 그 파일을 읽으며, 자리표시자만 담긴 `.env.dev.example`이 저장소에 함께 있다.
...
패키지 매니저로 설치하고(`brew install direnv` 또는 `sudo apt install direnv`), 셸 설정 파일 맨 아래에 `eval "$(direnv hook bash)"` 한 줄을 넣어 훅을 건다.
...
그래서 direnv는 갖추면 편한 도구이지 필수 도구가 아니다.
두 스크립트가 `.env.dev`를 스스로 읽으므로, direnv를 설치하지 않아도 개발 DB로 서버를 띄우고 마이그레이션을 적용하는 일은 그대로 된다.
direnv가 값을 하는 것은 스크립트로 만들 것까지는 없는 일회성 명령을 던질 때다.
`psql "$MIGRATE_DATABASE_URL"`로 테이블을 들여다보거나 `go run ./cmd/migrate`를 그대로 실행하는 경우가 그렇다.
설치할 도구를 하나 늘리는 값이 있는지는 각자의 작업 방식이 정할 문제이므로, 이 저장소는 direnv를 쓰는 사람만 `.envrc` 한 줄을 두면 되도록 해 두었다.

덧붙여 Go에는 `.env` 파일을 읽는 관례가 없다는 점도 알아 두면 좋다.
표준 라이브러리가 제공하는 것은 `os.Getenv`뿐이고, 이 앱이 godotenv 같은 라이브러리를 넣지 않은 것은 앞 절에서 본 대로 배포 환경이 값을 프로세스에 직접 주입하기 때문이다.
앱이 파일을 읽지 않기로 했다면 셸에 값을 올리는 일은 앱 바깥의 누군가가 맡아야 하고, direnv는 언어와 무관한 셸 계층에서 그 자리를 메운다.
설정을 환경 변수로 받는 12요소 앱(Twelve-Factor App)의 원칙을 앱 코드에서 지키면서, 로컬의 편의는 셸에서 해결하는 역할 분담이다.
