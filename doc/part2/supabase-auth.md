...
이 장의 핵심 주장은 둘이다.
로그인 상태를 브라우저 스크립트가 닿을 수 없는 곳(HttpOnly 쿠키)에 두면 공격면 하나가 통째로 사라진다는 것.
그리고 서버가 세션 저장소를 두지 않는 무상태 구조가, 인스턴스가 언제든 사라지는 서버리스와 정확히 맞물린다는 것이다.
...
둘째, 소셜 로그인 연동이다.
Google과 GitHub 각각에 OAuth 앱을 등록하고, 인가 코드(authorization code)를 토큰으로 교환하는 절차를 구현하고, 프로바이더마다 미묘하게 다른 응답 형식과 오류 케이스를 처리해야 한다.
...
그러나 Echo Flip의 화면은 9장과 10장에서 본 대로 서버에서 렌더링되고, 곧 확인하겠지만 로그인 흐름에도 브라우저 자바스크립트가 필요 없다.
...
그 주소를 만드는 쪽은 `internal/web/gotrue.go`의 `authorizeURL`이다.
...
이 책에서는 쿠키로 유지되는 로그인 상태를 로그인 세션이라 부르는데, 11장에서 말한 세션(Claude Code와 나누는 대화 한 판)과는 전혀 다른 뜻이다.
:::
...
공개 페이지와 로그인 필수 페이지가 미들웨어 사슬로 갈라지는 곳이다.
...
상세 절차는 저장소의 DEPLOY.md 3단계에 정리되어 있으니, 여기서는 구조만 잡는다.

첫 번째 층은 프로바이더 쪽 등록이다.
Google Cloud Console과 GitHub Developer Settings 각각에 OAuth 앱을 만들고, 발급받은 Client ID와 Secret을 Supabase 대시보드의 Providers 설정에 입력한다.
...
실제로 로그인 후 다시 로그인 화면으로 돌아오는 증상의 가장 흔한 원인이 이 목록에 배포 URL을 빠뜨린 경우다.
