...
::: info [용어 풀이] 액세스 토큰(Access Token)과 리프레시 토큰(Refresh Token)
액세스 토큰은 요청을 보낼 때마다 내미는 단기 출입증이고, 리프레시 토큰은 그 출입증이 만료됐을 때 새것을 받아 오는 데 쓰는 장기 증서다.
수명을 이렇게 둘로 나누는 이유는 탈취에 대비하기 위해서다.
...
`isHTTPS`가 조건부인 이유는 운영 구성을 로컬(`http://localhost:8080`)에서 확인할 때도 로그인이 돌아야 하기 때문이고, Vercel의 프록시(방문자의 요청을 먼저 받아 함수에 전달하는 중계 서버) 뒤에서는 `X-Forwarded-Proto` 헤더로 원래 요청이 HTTPS였는지 판별한다.
...
::: info [용어 풀이] CSRF(교차 사이트 요청 위조, Cross-Site Request Forgery)
...
검증하는 쪽은 이 항목들을 읽어 요청의 주인을 정하므로, 어떤 클레임을 어디까지 믿을지 정하는 일이 곧 인증 규칙이 된다.
:::
...
::: info [용어 풀이] 대칭 키와 비대칭 키(Symmetric/Asymmetric Key)
...
::: info [용어 풀이] Client ID와 Client Secret
