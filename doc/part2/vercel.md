# 17장 Vercel: 한 플랫폼에 모두 배포하기

14장에서 로컬의 두 프로세스를 띄웠고, 16장에서 그 코드가 원격 검사를 통과하게 만들었다.
1부의 로컬 모드에서는 배포라는 단계 자체가 필요 없었지만, 이 장부터는 검증된 그 코드가 내 컴퓨터를 벗어나 세상으로 나간다.
...
아울러 15장에서 정한 main·release 브랜치 정책이 Vercel 위에서 개발 배포와 운영 배포로 실현되는 구성도 이 장에서 정한다.
...
첫째, 무료 티어의 지속성이 불안하다.
Koyeb, Fly.io, Render 같은 플랫폼의 무료 범위는 정책 변경이 잦았고, 한동안 요청이 없는 무료 인스턴스를 재워 두는 슬립을 도입하거나 무료 범위 자체를 축소하는 방향으로 바뀌는 경우가 많았다.
...
Vercel의 무료 요금제인 Hobby 플랜은 정적 호스팅과 글로벌 CDN, Go를 포함한 서버리스 함수 런타임, 자동 HTTPS와 커스텀 도메인, GitHub 연동 자동 배포를 결제 수단 등록 없이 제공한다.
상용 환경이라면 CDN, 인증서 관리, CI/CD 파이프라인으로 각각 따로 구축하거나 사야 할 것들이 무료 플랜에 기본으로 들어 있다.

구체적인 한도 수치와 초과했을 때 벌어지는 일, 그리고 Hobby 플랜을 개인·비상업 용도로 제한하는 약관은 Supabase 쪽 한도와 함께 21장에서 묶어 다룬다.
두 서비스 중 무엇이 먼저 한도에 닿는지는 한쪽만 봐서는 알 수 없기 때문이다.
Echo Flip의 실사용량을 대입하면 Vercel의 어떤 한도에도 근처조차 가지 않는다는 것만 미리 말해 둔다.
...
11장에서 다뤘듯 Echo Flip의 프런트엔드는 Next.js의 정적 export(static export)를 사용한다.
...
"프런트엔드는 무조건 정적으로"라는 결정이 콜드스타트 걱정의 범위를 API로만 좁혀 줬다.
...
빌드 절차를 적은 설정 파일 없이, Vercel이 저장소 루트에서 Next.js 프로젝트를 감지해 `next build`를 실행하고 `out/`을 배포한다.
...
다만 어느 브랜치의 push가 운영 반영이 되는지는 뒤의 "개발 배포와 운영 배포를 가른다" 절에서 정한다.
...
첫째, 코드 재사용이다.
7장에서 만든 Gin 라우터(`pkg/app`)를 그대로 쓰면 로컬 서버와 프로덕션이 완전히 같은 코드로 동작한다.
...
단일 함수의 실체를 보자.
들어온 요청을 Gin 라우터에 그대로 넘겨주는 짧은 연결 코드로, `api/index.go` 전체가 다음과 같다.
...
7장에서 봤듯 Gin의 `*gin.Engine`은 표준 `http.Handler` 인터페이스를 구현한다.
...
아래 코드는 7장에서 본 로컬 진입점의 뼈대로, `DATABASE_URL`이 설정된 PostgreSQL 모드에서는 Vercel과 같은 `app.Engine()`을 그대로 부른다.
...
```go
	...
	if cfg.Driver == "sqlite" {
		// ... 1부의 로컬 모드: litestore를 열어 app.New로 조립한다 (7장)
	} else {
		engine, err = app.Engine()
		if err != nil {
			log.Fatal(err)
...
`:path*`는 `/api/` 뒤에 무엇이 얼마나 길게 붙든 전부 들어맞는 패턴으로, 0개 이상의 경로 세그먼트에 매칭된다.
...
4장에서 다뤘듯 Go에는 `internal` 디렉터리 관례가 있다.
...
주석의 "(directly)"라는 단서가 이 구도를 정확히 표현한다.
...
13장에서 다룬 go-quality 서브에이전트의 검사 목록 마지막 항목이 바로 이것이다.
...
이 콜로케이션(colocation) 전략의 정량적 근거와 Supabase 쪽 설정은 19장에서 자세히 다룬다.

### rewrites: 정적 export에서 동적 경로 흉내 내기

rewrites의 첫 항목(`/api/:path*`)은 앞 절에서 다뤘으니, 나머지 네 항목을 보자.
덱이나 카드마다 달라지는 주소를 미리 만들어 둔 정적 페이지 한 장으로 이어 주는 규칙들이다.
...
11장에서 봤듯 Echo Flip의 해법은 "정적 페이지 하나 + 클라이언트에서 경로 해석"이다.
...
아래 한 줄은 프런트엔드가 API를 호출할 때 주소 앞부분을 무엇으로 삼을지 정하는 코드다.
...
개발 모드에서 로컬 API 서버(`localhost:8080`)를 가리키는 기본값은 14장에서 본 무설정 로컬 실행의 장치다.
프로덕션 빌드에서는 `NEXT_PUBLIC_API_URL`을 아예 설정하지 않으므로 `BASE`가 빈 문자열이 되고, `/api/decks` 같은 상대 경로 호출이 그대로 같은 오리진으로 나간다.
주소가 서로 다르면 브라우저는 본 요청에 앞서 보내도 되는지 묻는 사전 확인, 즉 CORS 프리플라이트(preflight) 요청을 먼저 보낸다.
같은 오리진에서는 이 절차가 아예 없으니 API 호출마다 왕복 하나가 절약되고, 7장에서 본 `pkg/app/app.go`의 CORS 미들웨어는 `ALLOWED_ORIGINS`가 설정된 로컬 개발에서만 활성화된다.
...
::: info [용어 풀이] 프리뷰 배포(Preview Deployment)
...
그런데 15장에서 정한 브랜치 정책은 `main`을 개발 브랜치로, `release`를 운영 브랜치로 쓰는 것이었다.
...
이 설정 하나로 15장의 브랜치 정책이 배포 흐름으로 실현된다.
...
16장에서 Vercel 빌드는 "만들 수 있는가"를, CI는 "옳은가"를 묻는다고 정리했는데, 프리뷰 배포는 "실제 인프라 위에서 돌아가는가"라는 세 번째 확인을 얹는다.

환경 변수도 같은 축으로 갈린다.
Vercel의 환경 변수에는 Production, Preview, Development라는 스코프(scope)가 있어서, 같은 변수 이름에 스코프별로 다른 값을 등록할 수 있다.
...
개발 데이터베이스와 운영 데이터베이스를 왜, 어떻게 분리하는지는 19장에서 자세히 다룬다.
...
개발용 프로젝트도 하나 더 만드는데, 그 이유와 구성은 19장의 몫이다.

둘째, 데이터베이스 마이그레이션을 실행한다.
`go run ./cmd/migrate`가 3장에서 다룬 스키마를 Supabase의 PostgreSQL에 적용한다.

셋째, OAuth 앱을 등록한다.
Google과 GitHub에 OAuth 애플리케이션을 만들고 자격 증명을 Supabase에 연결한다.
이 인증 흐름의 구조는 18장의 주제다.

넷째, GitHub 저장소를 만들어 push하고, 15장의 정책대로 운영용 `release` 브랜치를 만들어 둔다.

다섯째, Vercel에서 저장소를 import하고, 프로덕션 브랜치를 `release`로 바꾸고, 환경 변수를 등록한다.
Vercel이 Next.js 프로젝트를 자동 감지하므로 빌드 설정은 손댈 것이 없고, 사람이 입력할 것은 프로덕션 브랜치 설정과 환경 변수뿐이다.
...
| `NEXT_PUBLIC_API_URL` | (같은 오리진이므로 빈 값으로 두거나 아예 만들지 않기) |
...
`DATABASE_URL`이 왜 직접 연결(5432)이 아니라 트랜잭션 풀러(6543)여야 하는지는 서버리스의 연결 폭증 문제와 얽혀 있는데, 19장에서 자세히 다룬다.
등록할 때는 앞 절에서 본 대로 Production에는 운영 Supabase 프로젝트의 값을, Preview에는 개발 프로젝트의 값을 넣는다.

여기까지 마치면 이후의 배포는 전부 자동이다.
`git push origin main` 한 번에 Vercel이 정적 프런트엔드 빌드와 Go 함수 컴파일을 함께 수행해 개발 확인용 프리뷰 배포를 만들고, `main`을 `release`에 병합하면 같은 과정이 운영 배포로 실행된다.
성공하면 원자적으로 새 버전이 서비스되고, 배포 스크립트도, 배포 파이프라인 설정도, 서버 접속도 없다.
...
남은 축은 Supabase다.
다음 18장에서는 공개된 앱의 관문인 인증, 곧 OAuth와 JWKS 검증의 전체 그림을 다룬다.
함수 리전을 iad1에 고정한 이유였던 콜로케이션과 트랜잭션 풀러의 사정은 19장에서 이어진다.
