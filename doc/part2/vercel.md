...
넷째, 실행 시간 제한.
서버리스 함수에는 요청당 실행 시간 제한이 있다(무료 티어 기준 수 초에서 수십 초 수준이며, 플랫폼과 요금제에 따라 다르다).
...
4장에서 다뤘듯 Echo Flip의 프런트엔드는 Next.js의 정적 export(static export)를 사용한다.
...
Vercel은 이 결과물을 CDN(Content Delivery Network)에 올려 전 세계 엣지에서 서빙한다.
...
`main`이 아닌 브랜치에 push하거나 풀 리퀘스트를 열면 프리뷰 배포(Preview Deployment)가 만들어진다.
...
첫째, 코드 재사용이다.
2장에서 만든 Gin 라우터(`pkg/app`)를 그대로 쓰면 로컬 서버와 프로덕션이 완전히 같은 코드로 동작한다.
...
2장에서 봤듯 Gin의 `*gin.Engine`은 표준 `http.Handler` 인터페이스를 구현한다.
...
1장에서 다뤘듯 Go에는 `internal` 디렉터리 관례가 있다.
...
7장에서 다룬 go-quality 서브에이전트의 검사 목록 마지막 항목이 바로 이것이다.
...
이 콜로케이션(colocation) 전략과 Supabase 쪽 설정은 9장에서 자세히 다룬다.

`DEPLOY.md`도 이 결정을 배포 절차의 첫 단계에 못 박아 두었다.
...
"서울 리전을 고르면 안 됩니다"라는 문장이 직관에 반해서 좋다.
데이터베이스만 서울에 두면 사용자↔함수는 한 번의 횡단, 함수↔데이터베이스는 매 쿼리마다 횡단이 되어 최악의 조합이 된다.
...
4장에서 봤듯 Echo Flip의 해법은 "정적 페이지 하나 + 클라이언트에서 경로 해석"이다.
...
정적 프런트와 Go 함수가 같은 도메인에서 서빙되므로, 프로덕션에서 프런트엔드의 API 호출은 같은 오리진(same-origin) 요청이다.
...
CORS 프리플라이트(preflight) 요청이 발생하지 않으니 API 호출마다 왕복 하나가 절약되고, 2장에서 본 `pkg/app/app.go`의 CORS 미들웨어는 `ALLOWED_ORIGINS`가 설정된 로컬 개발에서만 활성화된다.
...
`go run ./cmd/migrate`가 5장에서 다룬 스키마를 Supabase의 PostgreSQL에 적용한다.

셋째, OAuth 앱을 등록한다.
Google과 GitHub에 OAuth 애플리케이션을 만들고 자격 증명을 Supabase에 연결한다.
이 인증 흐름의 구조는 9장의 주제다.
...
`DATABASE_URL`이 왜 직접 연결(5432)이 아니라 트랜잭션 풀러(6543)여야 하는지는 서버리스의 연결 폭증 문제와 얽혀 있는데, 9장에서 자세히 다룬다.
