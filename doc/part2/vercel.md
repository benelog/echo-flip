...
아래 코드는 그 엔진으로 포트를 열어 상주 서버처럼 도는 로컬 진입점이다.
...
그래서 Vercel 대시보드의 프로젝트 설정(Settings → Git → Production Branch)에서 프로덕션 브랜치를 `release`로 바꾼다.
메뉴 구성은 바뀔 수 있으니 경로보다 이 설정의 존재를 기억해 두면 된다.
...
성공하면 원자적으로 새 버전이 서비스되고, 배포 스크립트도, CI 설정 파일도, 서버 접속도 없다.
