...
Echo Flip의 API처럼 실행 시간 대부분이 데이터베이스 응답 대기인 워크로드는 요청에 걸린 전체 시간보다 훨씬 적게 소모한다.
...
매일 쓰는 사용자가 100명으로 늘어도 자릿수가 두 개 커질 뿐 여전히 한도 안이며, 그 전에 비상업 조건이나 15장에서 다룰 Supabase 무료 티어의 한도가 먼저 신호를 보낼 것이다.
...
9장에서 다뤘듯 Echo Flip의 프런트엔드는 Next.js의 정적 export(static export)를 사용한다.
...
첫째, 코드 재사용이다.
5장에서 만든 Gin 라우터(`pkg/app`)를 그대로 쓰면 로컬 서버와 프로덕션이 완전히 같은 코드로 동작한다.
...
5장에서 봤듯 Gin의 `*gin.Engine`은 표준 `http.Handler` 인터페이스를 구현한다.
...
이 콜로케이션(colocation) 전략의 정량적 근거와 Supabase 쪽 설정은 15장에서 자세히 다룬다.
...
9장에서 봤듯 Echo Flip의 해법은 "정적 페이지 하나 + 클라이언트에서 경로 해석"이다.
...
같은 오리진에서는 이 절차가 아예 없으니 API 호출마다 왕복 하나가 절약되고, 5장에서 본 `pkg/app/app.go`의 CORS 미들웨어는 `ALLOWED_ORIGINS`가 설정된 로컬 개발에서만 활성화된다.
...
`go run ./cmd/migrate`가 11장에서 다룬 스키마를 Supabase의 PostgreSQL에 적용한다.

셋째, OAuth 앱을 등록한다.
Google과 GitHub에 OAuth 애플리케이션을 만들고 자격 증명을 Supabase에 연결한다.
이 인증 흐름의 구조는 15장의 주제다.
...
`DATABASE_URL`이 왜 직접 연결(5432)이 아니라 트랜잭션 풀러(6543)여야 하는지는 서버리스의 연결 폭증 문제와 얽혀 있는데, 15장에서 자세히 다룬다.
