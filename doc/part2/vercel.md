# 16장 Vercel: 한 플랫폼에 모두 배포하기

14장에서 로컬의 두 프로세스를 띄웠고, 15장에서 그 코드가 원격 검사를 통과하게 만들었다.
이 장부터는 검증된 코드를 인터넷 위로 옮기는 인프라를 살펴본다.
...
Vercel의 무료 요금제인 Hobby 플랜은 정적 호스팅과 글로벌 CDN, Go를 포함한 서버리스 함수 런타임, 자동 HTTPS와 커스텀 도메인, GitHub 연동 자동 배포와 프리뷰 배포를 결제 수단 등록 없이 제공한다.
상용 환경이라면 CDN, 인증서 관리, CI/CD 파이프라인으로 각각 따로 구축하거나 사야 할 것들이 무료 플랜에 기본으로 들어 있다.

구체적인 한도 수치와 초과했을 때 벌어지는 일, 그리고 Hobby 플랜을 개인·비상업 용도로 제한하는 약관은 Supabase 쪽 한도와 함께 20장에서 묶어 다룬다.
...
이 콜로케이션(colocation) 전략의 정량적 근거와 Supabase 쪽 설정은 18장에서 자세히 다룬다.
...
`DATABASE_URL`이 왜 직접 연결(5432)이 아니라 트랜잭션 풀러(6543)여야 하는지는 서버리스의 연결 폭증 문제와 얽혀 있는데, 18장에서 자세히 다룬다.
