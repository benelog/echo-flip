...
| 장 | 제목 | 파일 | 상태 |
|---|---|---|---|
| 도입 | 무엇을 만드는가: Echo Flip의 기능 요구사항 | `doc/intro.md` | 검토 완료 |
| 1부 1장 | 기술 선택: 요구사항에서 아키텍처까지 | `doc/part1/tech-choices.md` | 검토 완료 |
| 1부 2장 | Claude Code: AI 에이전트와 개발하기 | `doc/part1/claude-code.md` | 검토 완료 |
| 1부 3장 | 에이전트에게 지시하기: Plan 모드 활용 | `doc/part1/instructing.md` | 검토 완료 |
...
| 1부 7장 | Go 코드 읽기: 구조체, 포인터, 에러 처리 | `doc/part1/go.md` | 검토 완료 |
| 1부 8장 | Go 테스트와 품질 게이트: 도구, 훅, 서브에이전트 | `doc/part1/go-testing.md` | 검토 완료 |
| 1부 9장 | Gin으로 만드는 HTTP API | `doc/part1/gin.md` | 검토 완료 |
| 1부 10장 | HTML과 CSS: 화면을 이루는 문서와 스타일 | `doc/part1/html-css.md` | 검토 완료 |
| 1부 11장 | html/template으로 만드는 화면 | `doc/part1/go-templates.md` | 검토 완료 |
| 1부 12장 | htmx: 자바스크립트 없이 만드는 동적 화면 | `doc/part1/htmx.md` | 검토 완료 |
| 1부 13장 | 로컬 개발 환경: 내 컴퓨터에서 앱 완성하기 | `doc/part1/local-dev.md` | 검토 완료 |
| 2부 14장 | Git: 개념과 브랜치 정책 | `doc/part2/git.md` | 검토 완료 |
| 2부 15장 | GitHub Actions: 원격 품질 게이트 | `doc/part2/github-actions.md` | 검토 완료 |
| 2부 16장 | Vercel: 한 플랫폼에 모두 배포하기 | `doc/part2/vercel.md` | 검토 완료 |
| 2부 17장 | Supabase 인증: OAuth와 JWKS 검증 | `doc/part2/supabase-auth.md` | 검토 완료 |
| 2부 18장 | Supabase 데이터베이스: pgx 연결과 개발·운영 DB 분리 | `doc/part2/supabase-db.md` | 검토 완료 |
| 2부 19장 | PWA: 설치되는 앱으로 만들기 | `doc/part2/pwa.md` | 검토 완료 |
| 2부 20장 | 무료 티어 운영과 한도 관리 | `doc/part2/free-tier.md` | 검토 완료 |
| 2부 21장 | 다음 단계: 여기서 더 공부할 것들 | `doc/part2/whats-next.md` | 검토 완료 |
| 부록 A | 개발 도구 설치 | `doc/appendix/setup.md` | 검토 완료 |

상태 값: `미착수` → `초안 작성 중` → `초안 완료` → `검토 완료`
2026-07-11 Go SSR 전환 재편에서 신설·재작성·수정된 장은 `초안 완료`로 두었다가, 같은 날 비개발자 풀어쓰기·구체화 중심 통독 검토(아래 로드맵 마지막 항목)를 통과해 전 장을 `검토 완료`로 승격했다.
...
- **13장 로컬 개발 환경** (1부 피날레): 준비물은 Go뿐, 명령 하나(`go run ./cmd/server`, :8080)로 로그인 없이 앱 완성, 무설정 실행의 동작 원리(config.go 분기, LocalMiddleware, 로컬 모드에선 로그인 화면 자체가 안 뜸), 내 데이터는 파일 하나(litestore.Open의 스키마 자동 적용, sqlite3 실습, 백업=파일 복사), `.vscode/` 네 파일 해부(extensions/settings/tasks/launch, launch.json은 환경 변수 없이 F5), 사람의 입구와 에이전트의 입구, 환경 변수로 동작 바꾸기(SQLITE_PATH·PORT, 운영 연결·direnv는 18장), 1부 마무리와 2부 예고.
...
- **18장 Supabase 데이터베이스**: SQLite에서 PostgreSQL로(DATABASE_URL이 모드를 정한다), 같은 설계 더 풍부한 타입, pgx와 서버리스 커넥션 폭증, 트랜잭션 풀러(6543)와 simple protocol, 마이그레이션, 리전 콜로케이션, RLS 전략(정책 0개, 17장 접점), 개발 DB와 운영 DB 분리(3층 구조, 브랜치·배포 스코프 배선, 무료 활성 프로젝트 2개 한도), 환경 변수 구성(값이 두 벌, anon key가 비밀이 아닌 이유, direnv와 .envrc). 마이그레이션·RLS·환경 변수 용어 상자를 이 장이 소유.
- **19장 PWA**: 설치 조건 3가지(HTTPS·매니페스트·서비스 워커), 정적 `internal/web/static/manifest.webmanifest`(display standalone, start_url 등), 아이콘 3개와 maskable, `sw.js`(v2) 해부: 페이지 HTML은 서버 렌더링이라 network-first(오프라인일 때만 캐시), 정적 자산은 stale-while-revalidate, `/api/`·비GET·외부 오리진은 network only, skipWaiting+clients.claim, `CACHE = "echo-flip-v2"`를 손으로 올리는 구조와 Workbox를 안 쓴 이유, 등록은 app.js(10장), 포기한 것(오프라인 학습·푸시·백그라운드 동기화), DevTools Application 탭 확인.
...
| 파일 | 소유 용어 |
|---|---|
| `intro.md` | API (구 배정 상자들은 2026-07-11 재편으로 비기능 요구사항과 함께 1장으로 이동) |
| `part1/tech-choices.md` | 컴파일/인터프리터 언어, 런타임, 정적/동적 타입, 프레임워크와 라이브러리, 의존성, 관계형 데이터베이스, 호스팅, 무료 티어, PostgreSQL, 프런트엔드와 백엔드, PWA, 서버리스, 콜드스타트, 아키텍처, OAuth, JWT (도입 소속 장이라 예외적으로 16개, 구 도입 상자를 흡수한 이력. 간격 반복·트레이드오프는 일반 개념이라 상자 없이 본문 풀어쓰기) |
...
| `part1/go-testing.md` | 테이블 주도 테스트, 포매터와 정적 분석, 품질 게이트, 훅(Claude Code), 종료 코드, 서브에이전트 (뒤 4개는 구 12장 훅 장의 8장 통합으로 이동. 세로 정렬은 상자 없이 본문 절로 다룸) |
...
**훅(Hook)은 동음이의어다.** 8장의 훅은 Claude Code가 자동 실행하는 스크립트이고, React 같은 프런트엔드 프레임워크의 훅(상태 관리 함수)과는 다른 것이다.
8장의 상자에서 이 구분을 한 문장으로 짚는다. "세션"도 동음이의어다: 2장의 세션은 Claude Code의 대화 단위, 17장의 로그인 세션은 인증 상태다(17장 쿠키 상자에서 구분).
...
- [x] 이전부터 남은 통독 검토: 5·6·8·9·10·20장, 부록 A → 8·9·10·11장은 아래 개편으로 폐기되어 검토 불필요, 나머지는 2026-07-11 전 장 통독 검토(로드맵 마지막 항목)에 포함해 완료
- [x] 2부 장 수 보강 완료 (2026-07-10): 제안한 3개 장(GitHub Actions·무료 티어·PWA)을 모두 신설하고 Supabase를 분할해 2부가 6장 → 10장이 됐다. 1부 11장 대 2부 10장.
- [x] (선택) 부록(로컬 개발 환경·배포 절차) 추가 검토 → 추가하지 않기로 결정 (2026-07-11 저자 확인): 로컬 개발 환경은 13장이, 배포 절차는 16~18장과 저장소 DEPLOY.md가 이미 다뤄 중복이라는 판단.
...
- [x] **책 재편: TypeScript·React를 빼고 HTML을 넣는다** (2026-07-11 지시, 같은 날 완료. 병렬 집필 에이전트 9개):
  ...
  - 남은 일이었던 재편 장 통독 검토는 아래 항목으로 완료. 화면 캡처 재촬영만 남았다.
- [x] 화면 캡처 재촬영: Go SSR 전환 후 재배포된 화면으로 그림 1~4를 다시 찍는다(UI가 거의 동일하게 재현됐지만 재배포 후 다시 찍는 것이 안전. 캡처 조건은 위 "화면 캡처" 절).
  - 2026-07-11 저자 결정: 로컬 모드 촬영 대신 **배포본 수리 후 촬영**. 현재 배포본(echo-flip-delta.vercel.app)은 500 오류(`server misconfigured: SUPABASE_URL and SUPABASE_ANON_KEY are required`) 상태다. 선행 조건은 사용자 몫 수동 단계: Vercel 대시보드에서 `SUPABASE_URL`·`SUPABASE_ANON_KEY`를 Production(필요하면 Preview도) 스코프에 등록하고 재배포. 배포본이 살아나면 로그인 후 데모 덱("TOEIC 필수 단어" 8장)을 준비해 촬영한다.
  - 2026-07-11 완료: 수리된 배포본에서 그림 1~4 재촬영·교체. 방법: 별도 프로필 Chrome(디버깅 포트)에 저자가 직접 로그인 → puppeteer-core가 CDP로 붙어 430×860 뷰포트·3배 배율 촬영(채점 버튼은 누르지 않아 SRS 데이터 무변경). 학습 3단계의 카드는 "call it a day"(숙어)로 바뀜. 폐기된 그림 5 파일(flashcard-pair.png)도 삭제. 검증: `npm run build` 통과.
- [x] **전 장 통독 검토: 비개발자 풀어쓰기·추상 표현 구체화 중심** (2026-07-11 지시, 병렬 검토 에이전트 10개): `초안 완료`이던 19개 장(도입·1~3·7~13·15~21장·부록 A)을 두 관점(비개발자에게 풀어 설명할 곳, 추상·모호 표현의 구체화)으로 검토·수정하고 전부 `검토 완료`로 승격. 수정 약 60곳.
  - 유형별: 코드 블록 앞 "이 코드가 하는 일" 예고 문장 보강 15곳, 첫 등장 용어의 일상어 풀이(싱글턴·YAML·크론·V8/WASM·공격면·CRUD·WSL·gitleaks, "사상한다"→"대응시킨다" 등), 정도 부사("압도적으로"·"훨씬"·"차이가 크다")를 근거나 실측 수치로 교체(app.js "몇십 줄"→81줄 등), 지시 대상이 먼 "이 구조"에 지시 대상 명시, 금지 표현 잔존분 정리(갈라지다·갈리다 계열 6곳).
  - 사실 교정 2건: 2장의 "이 저장소에는 CLAUDE.md가 없다" 절을 실제 루트 CLAUDE.md 전문 인용으로 재작성(직전 통독 검토에서 파일을 추가해 서술이 거짓이 됐던 것. 가상 예시 코드 블록은 삭제하되, api/index.go 제약을 문서 아닌 서브에이전트가 강제한다는 논지는 유지), 21장 "마치며"의 1부 회고가 개편 전 장 순서를 따르던 잔재 재배열. pwa.md "'데이터'가 /api/로 끝나지 않는다"(경로는 /api/로 시작), html-css.md "app.css 맨 위"(실제 48행) 교정 포함.
  - 이 파일(book-plan.md)의 낡은 기록 갱신: 용어 배정표를 실물 기준으로(도입 상자들의 1장 이동 반영, 폐기된 agents-hooks.md 행을 8장으로 통합, 세로 정렬은 상자 아닌 본문 절), 동음이의어 문단 장 번호(12→8장 훅, 11→2장 세션), 그림 5 폐기 반영.
  - 검토 에이전트가 보류·보고만 한 것(사실 지어내기 금지 원칙): Vercel "GB-시간" 단위의 일상어 풀이(요금 문서 재확인 필요), PWA 아이콘 192/512px의 용처 구분, 17장 "로그인 루프의 흔한 원인은 배포 URL 누락" 같은 경험적 주장. 부록 A의 Go·Git 첫 사용 시점을 13장으로 둘지 8장(직접 실행 독자)으로 당길지는 저자 판단 대기.
  - 보류분 해소 (2026-07-11, 같은 날): GB-시간은 Vercel 요금 문서(functions/usage-and-pricing, 2026-06-16 갱신) 재확인 후 20장에 풀이 문단 추가(메모리 크기 × 인스턴스 생존 시간, I/O 대기 중에도 쌓이고 요청이 끝나면 인스턴스가 멈춰 소모도 멈춤, 360GB-시간=1GB 기준 360시간). PWA 아이콘은 Chrome 설치 조건 문서(web.dev) 확인 후 19장에 용처 구체화(192px=홈 화면, 512px=시작 화면, 나머지 크기는 확대·축소). 17장 로그인 루프 문장은 저자 판단으로 완화("가장 흔한 원인이 ~다" → "증상을 만나면 ~부터 확인해 볼 만하다"). 부록 A의 Git·Go 첫 사용 시점은 저자 판단으로 13장 유지(주 흐름은 6~8장에서 읽고 13장에서 처음 실행한다는 관점).
  - 검증: 검토 전 스냅샷과 대조해 19개 파일의 코드 펜스 전수 바이트 동일(2장 CLAUDE.md 절 재작성은 그 후 별도 수행), 분량 증가 최대 104.6%(상한 110%), 발췌·수치는 에이전트가 저장소 실물과 대조(gotrue.go 125줄, Store 인터페이스 32개 메서드, app.js 81줄, htmx.min.js 49.9KB, schema.sql의 if not exists 전수 등 일치), `npm run build` 통과.
