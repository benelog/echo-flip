...
4장에서 본 정규화가 중복을 걷어 내는 작업이라면, 비정규화(denormalization)는 조회를 쉽게 하려고 중복을 일부러 남겨 두는 반대 방향의 선택이다.
...
갱신 빈도가 다른 데이터를 분리하면 복습 한 번의 갱신이 카드 본문 행을 건드리지 않으니, 여러 요청이 동시에 도는 운영 DB에서는 잠금이 겹쳐 기다리는 범위(잠금 경합)도 좁아진다.
...
규칙의 값들이 SQL 문자열에 직접 이어 붙지 않고 자리표시자 `?`로 바인딩되는 것은 4장에서 본 SQL 인젝션 방지 원칙 그대로다.
...
값을 읽어 와 Go에서 더한 뒤 다시 쓰는 방식과 달리 DB가 한 번에 처리하므로 중간에 다른 요청이 끼어들 틈이 없고(이런 성질을 원자적이라고 한다), 서버와 DB를 오가는 횟수도 한 번으로 준다.
