...
이 쿼리는 사용자가 딱 하나로 정해지는 조건(`user_id` 등치) 뒤에 복습 시각이 일정 시점 이전이라는 조건(`due_at` 범위)이 오는 형태라, 2장에서 본 (등치, 범위) 순서 원칙에 정확히 들어맞고 `order by due_at`까지 인덱스 순서로 해결할 수 있다.
...
반면 `DueCards`의 실행 계획(DB가 쿼리를 처리하는 경로)을 `explain query plan`으로 확인해 보면 이 인덱스를 타지 않는다.
