# 11장 PostgreSQL 데이터베이스 설계

애플리케이션의 수명은 코드보다 데이터가 길다.
프레임워크는 갈아탈 수 있어도 쌓인 학습 기록은 버릴 수 없기 때문에, 데이터베이스 설계는 앱 전체에서 가장 신중해야 할 결정이다.
관계형 데이터베이스와 PostgreSQL을 선택한 이유는 1장에서 정리했고, 테이블과 키·SQL·인덱스의 기초는 10장에서 다뤘다고 전제한다.
...
인증 시스템의 식별자와 앱 데이터의 식별자를 일치시키면 조인 없이 JWT의 사용자 ID로 바로 조회할 수 있다(인증 흐름은 15장에서 다룬다).
...
갱신 빈도가 다른 데이터를 분리하면 복습 트랜잭션이 카드 본문 행을 건드리지 않으니, 잠금이 겹쳐 기다리는 범위(잠금 경합)도 좁아진다.
...
`internal/db/migrations/000002_sharing.up.sql`에서 발췌한 다음 코드는 공유 slug가 있는 덱들 사이에서만 slug가 겹치지 않게 막는다.
...
이 쿼리는 사용자가 딱 하나로 정해지는 조건(`user_id` 등치) 뒤에 복습 시각이 일정 시점 이전이라는 조건(`due_at` 범위)이 오는 형태라, 10장에서 본 (등치, 범위) 순서 원칙에 정확히 들어맞고 `order by due_at`까지 인덱스 순서로 해결된다.

둘째, `cards_tags_gin`은 GIN(Generalized Inverted Index) 인덱스로, 스마트 덱 태그 규칙의 `tags && $2` 배열 연산을 받친다.
10장에서 본 B-tree 인덱스는 한 칸에 값 하나가 든 경우를 정렬해 찾는 구조라 배열 안의 개별 원소는 훑지 못한다.
...
행 잠금은 먼저 잡은 쪽의 작업이 끝날 때까지 다른 작업이 그 행을 고치지 못하게 세워 두는 장치다.
덕분에 같은 카드에 대한 판정 요청이 동시에 들어와도(더블 클릭, 재시도 등) 읽기-계산-쓰기 사이에 다른 트랜잭션이 끼어들 수 없다.
...
자문 잠금은 특정 행이 아니라 약속된 이름 하나를 잠그는 기능으로, "지금 누군가 마이그레이션 중"이라는 팻말을 DB에 걸어 두는 용도다.
주석의 "direct connection"은 이 자문 잠금 때문에 생기는 Supabase 특유의 제약으로, 왜 트랜잭션 풀링 포트를 거치면 안 되는지는 15장에서 다룬다.
...
덱 테이블에 공유용 열 두 개를 더하고, 앞 절에서 본 부분 유니크 인덱스로 공유 slug의 중복을 막는다.
...
덱의 순번(`seq`)을 받아 뒤섞인 4자 코드로 바꾸는 함수다.
...
Supabase의 PostgreSQL에는 PostgREST라는 관문 서버(게이트웨이)가 붙어 있다.
덕분에 브라우저 같은 클라이언트가 중간에 API 서버를 두지 않고도 웹 요청으로 테이블을 직접 읽고 쓸 수 있으며, 이때 접속자는 `anon`(로그인 전)과 `authenticated`(로그인 후)라는 두 역할 중 하나로 취급된다.
일반적인 DB 서버에는 없는 문이 하나 더 있다는 점이 다르다.
...
PostgreSQL 뷰는 기본적으로 소유자 권한으로 실행된다.
조회하는 사람이 누구든 뷰를 만든 계정의 권한으로 기반 테이블을 읽는다는 뜻이라, 테이블에 걸어 둔 RLS 잠금이 뷰를 거치는 순간 우회될 수 있다.
...
반면 누가 어떤 데이터에 손댈 수 있는지 판정하는 인가(authorization) 로직이 SQL 정책으로 흩어져 테이블마다 빠짐없이 작성·검증해야 하고, 정책 하나를 빠뜨리면 그대로 데이터 노출이라는 비용이 있다.
...
이 구조에서 Supabase는 인증 제공자 겸 관리형 PostgreSQL로만 쓰이는데, 그 연결 구성은 15장에서 이어서 다룬다.
...
다음 장부터는 2부로 넘어가, 12장의 Claude Code를 시작으로 이 앱의 코드를 만들어 낸 AI 에이전트와 앱이 돌아가는 인프라를 살펴본다.
