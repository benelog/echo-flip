...
인증 시스템의 식별자와 앱 데이터의 식별자를 일치시키면 조인 없이 JWT의 사용자 ID로 바로 조회할 수 있다(인증 흐름은 13장에서 다룬다).
...
복합 인덱스는 두 열 이상을 묶어 만든 하나의 찾아보기로, 성으로 먼저 정렬하고 같은 성 안에서 이름으로 정렬한 명부와 같다.
이 쿼리는 사용자가 딱 하나로 정해지는 조건(`user_id` 등치) 뒤에 복습 시각이 일정 시점 이전이라는 조건(`due_at` 범위)이 오는 형태라 (등치, 범위) 순서의 복합 인덱스가 정확히 들어맞고, `order by due_at`까지 인덱스 순서로 해결된다.
...
주석의 "direct connection"은 이 자문 잠금 때문에 생기는 Supabase 특유의 제약으로, 왜 트랜잭션 풀링 포트를 거치면 안 되는지는 13장에서 다룬다.
...
이 구조에서 Supabase는 인증 제공자 겸 관리형 PostgreSQL로만 쓰이는데, 그 연결 구성은 13장에서 이어서 다룬다.
...
다음 장부터는 2부로 넘어가, 10장의 Claude Code를 시작으로 이 앱의 코드를 만들어 낸 AI 에이전트와 앱이 돌아가는 인프라를 살펴본다.
