# 3장 데이터베이스 설계: 요구사항에서 테이블로

애플리케이션의 수명은 코드보다 데이터가 길다.
프레임워크는 갈아탈 수 있어도 쌓인 학습 기록은 버릴 수 없기 때문에, 데이터베이스 설계는 앱 전체에서 가장 신중해야 할 결정이다.
관계형 데이터베이스를 고르고 1부의 로컬 개발에는 SQLite를, 2부의 운영에는 PostgreSQL을 쓰는 이유는 1장에서 정리했고, 테이블과 키·SQL·인덱스의 기초는 2장에서 다뤘다고 전제한다.
이 장에서는 도입에서 정리한 요구사항이 실제 테이블과 인덱스로 구체화되는 과정을, 1부의 앱이 쓰는 SQLite 스키마를 근거로 따라가 보겠다.
간격 반복(Spaced Repetition) 상태를 저장하는 데이터 모델과 URL slug 설계까지가 이 장의 범위이고, 같은 설계가 운영 PostgreSQL에서 어떤 전용 타입과 장치로 표현되는지는 19장에서 다룬다.
...
인증 시스템의 식별자와 앱 데이터의 식별자를 일치시키면 조인 없이 인증된 사용자 ID로 바로 조회할 수 있다(운영의 인증 흐름은 18장에서 다룬다).
...
DB는 어떤 행을 고치는 동안 그 행을 잠가 다른 작업이 끼어들지 못하게 하므로, 자주 바뀌는 값과 거의 안 바뀌는 값이 한 행에 섞여 있으면 서로 기다리는 일이 잦아진다.
갱신 빈도가 다른 데이터를 분리하면 복습 트랜잭션이 카드 본문 행을 건드리지 않으니, 여러 요청이 동시에 도는 운영 DB에서는 잠금이 겹쳐 기다리는 범위(잠금 경합)도 좁아진다.
...
도입에서 정리했듯 재도전 라운드의 판정은 기록은 남기되 SRS에는 반영하지 않는데, 그 구분이 컬럼 하나로 스키마에 새겨져 있어 통계 쿼리들이 `is_retry = 0` 조건으로 첫 판정만 골라낸다.
...
"오답률 0.4 이상" 같은 규칙 조건과 store 계층(DB를 읽고 쓰는 Go 코드로, 5장과 7장에서 다룬다)의 카드 조회가 모두 이 뷰 하나를 바라보므로, 오답률 계산식이 한 곳에만 존재한다.
...
규칙의 값들이 SQL 문자열에 직접 이어 붙지 않고 자리표시자 `?`로 바인딩되는 것은 2장에서 본 SQL 인젝션 방지 원칙 그대로다.
...
이 스키마의 기본 키는 두 종류다.
사용자에게 노출될 수 있는 엔티티(덱, 카드, 세션 등)는 UUID를 담은 `text primary key`이고, 내부 로그인 `review_logs`만 `integer primary key autoincrement`라는 순번이다.
...
운영 스키마도 같은 전략인데, 그쪽은 uuid 타입에 `gen_random_uuid()` 기본값, 순번에는 identity 컬럼이라는 전용 장치가 있다는 점만 다르다(19장).
...
이 쿼리는 사용자가 딱 하나로 정해지는 조건(`user_id` 등치) 뒤에 복습 시각이 일정 시점 이전이라는 조건(`due_at` 범위)이 오는 형태라, 2장에서 본 (등치, 범위) 순서 원칙에 정확히 들어맞고 `order by due_at`까지 인덱스 순서로 해결된다.
...
`"2006-01-02T15:04:05.000Z"`라는 값은 Go가 예시 날짜로 시각 형식을 지정하는 독특한 방식인데, 이 문법은 5장에서 다룬다.
...
SQLite에는 시간대 변환 기능이 없으므로, `internal/litestore/stats.go`의 `DailyStats`는 UTC 텍스트로 쌓인 복습 시각을 읽어 와 Go에서 사용자의 IANA 시간대(예: `Asia/Seoul`)의 날짜로 묶는다.
...
넷째, 정답/오답 카운터 갱신을 `correct_count = correct_count + ?` 형태로 SQL 안에서 처리해, 읽어 온 값에 더해 쓰는 방식보다 원자적이고 왕복이 적다.
...
뷰를 통하지만 조건과 정렬이 모두 `card_srs` 쪽 컬럼이라, DB가 쿼리를 처리하는 경로(실행 계획)는 앞서 만든 `card_srs_user_due_idx (user_id, due_at)`를 타고 정렬 없이 상위 N건을 끊어 온다.
한편 홈 화면 배지에 쓰이는 같은 파일의 `DueCount`는 카드 본문이 필요 없으므로 뷰를 거치지 않고 `select count(*) from card_srs where user_id = ? and due_at <= ?`로 `card_srs`만 센다.
...
지금까지의 최대 순번에 1을 더하는 `max(seq) + 1` 채번이다.
...
"덱마다 순번 하나"라는 같은 설계가 환경의 동시성 수준에 따라 다른 채번 장치로 구현되는 대비로, 운영 쪽 장치는 19장에서 본다.
...
Echo Flip은 PostgreSQL의 행 수준 보안(RLS, Row Level Security)을 그 잠금 장치로 쓴다.
고치는 절차와 지키는 장치, 이 두 이야기는 운영 스키마(`internal/db/migrations/`)와 함께 19장에서 다룬다.
...
이 운영 이야기는 PostgreSQL의 전용 타입들과 함께 19장에서 이어진다.

여기까지가 데이터가 머무는 바닥층이다.
다음 4장부터는 한 층 올라가, 이 테이블들을 실제로 읽고 쓰는 백엔드 언어 Go의 기초를 익힌다.
