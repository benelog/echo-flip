# 3장 Go 코드 읽기: 구조체, 포인터, 에러 처리

2장에서 Go 코드가 담기는 틀(모듈과 디렉터리)과 변수, 함수, 제어문, 상수를 익혔다.
...
`const` 블록에는 2장에서 본 공개 규칙이 그대로 적용된다.
...
2장에서 본 대로 초기화하지 않은 값은 타입별 제로값(Zero Value)을 가지므로, `IntervalDays`와 `Repetitions`는 자동으로 `0`이 된다.
...
2장에서 다중 반환은 "결과와 에러"의 짝으로 만났는데, 여기서는 실패할 일이 없는 계산이라 에러 대신 서로 붙어 다니는 결과 두 개를 함께 돌려주는 데 쓰였다.
...
2장에서 본 대로 각 `case`는 자동으로 끝나므로 `break` 없이 깔끔하게 읽힌다.
...
현재 시각을 `time.Now()`로 직접 얻지 않고 `now` 인자로 받는 것도 같은 이유인데, 4장에서 테스트를 읽으며 효과를 확인하겠다.
...
HTTP 핸들러는 `store.ErrNotFound`를 받으면 404로 응답하는데, 이 연결은 5장에서 확인한다.
...
2장 config.go의 `Load`가 반환하던 `*Config`도 이제 읽을 수 있다.
...
2장 config.go에서 이미 쉼표로 자르는 `strings.Split`과 앞뒤 공백을 걷어 내는 `strings.TrimSpace`를 봤다.
...
5장에서 `c.ShouldBindJSON`과 `c.JSON`이라는 이름으로 다시 만나게 된다.
