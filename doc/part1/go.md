...
::: info [용어 풀이] 에러 래핑과 센티널 에러(Error Wrapping·Sentinel Error)
...
감싼 에러 안에 그 표지가 들어 있는지는 `errors.Is`가 사슬을 끝까지 따라가며 확인해 주므로, 맥락을 덧붙이면서도 원인을 알아보는 일이 함께 된다.
:::
...
`ErrNotFound`는 "요청한 행이 없다"를 뜻하는 store 패키지 공용 에러 값이다.
...
pgx 드라이버가 돌려준 `pgx.ErrNoRows`를 `errors.Is`로 알아본 뒤 `ErrNotFound`로 바꿔 돌려주므로, store 패키지를 쓰는 쪽은 드라이버가 무엇인지 몰라도 된다.
HTTP 핸들러는 `store.ErrNotFound`를 받으면 404로 응답하는데, 이 연결은 9장에서 확인한다.
...
::: info [용어 풀이] 메서드와 리시버(Method·Receiver)
...
슬라이스의 전형적인 사용 패턴은 internal/store/decks.go의 `ListDecks`에서 볼 수 있다.
...
::: info [용어 풀이] 직렬화와 역직렬화(Serialization·Deserialization)
