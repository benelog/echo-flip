...
가운데의 `if` 분기는 설정에 따라 SQLite 파일로 도는 로컬 모드와 PostgreSQL 모드를 가르는데, 그 사정은 7장과 13장에서 다루므로 지금은 문법의 재료로만 쓰겠다.
...
구조체는 5장의 주인공이니 여기서는 "이름표가 붙은 칸이 열 개 있는 상자" 정도로 이해하고 넘어가자.
...
각 모드에서 무엇이 달라지는지는 7장과 13장의 몫이다.
