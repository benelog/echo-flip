...
2장과 3장에서 데이터가 어떤 표에 어떤 모양으로 담기는지를 봤다.
...
이 장의 중심 예제는 환경 변수를 읽어 서버 설정을 만드는 internal/config/config.go로, 40줄짜리 파일 하나에 이 장의 문법 요소가 거의 다 들어 있다.
여기서 다진 기초는 다음 5장에서 간격 반복(Spaced Repetition) 알고리즘을 구현한 internal/srs 패키지를 비롯한 도메인 코드를 본격적으로 읽기 위한 준비 운동이다.
...
cmd/server/main.go의 핵심 부분으로, 앱을 조립해 지정한 포트에서 서버를 띄우는 시작 코드다.
...
파일의 앞부분으로, 설정값 다섯 개를 한 묶음으로 담을 그릇을 정의한다.
...
구조체는 5장의 주인공이니 여기서는 "이름표가 붙은 칸이 다섯 개 있는 상자" 정도로 이해하고 넘어가자.
대신 각 칸(필드) 이름 뒤에 붙은 타입 표기를 눈여겨보자.

`string`은 문자열 타입이다.
데이터베이스 주소도, 포트 번호도 여기서는 글자들의 나열로 다룬다.
`[]string`은 문자열 슬라이스(Slice)로, 문자열을 담는 가변 길이 목록이다.
접속을 허용할 웹 주소(`AllowedOrigins`)는 여러 개일 수 있어 목록으로 선언했다.
슬라이스의 동작 원리는 5장에서 자세히 본다.

이 파일에는 등장하지 않지만 자주 쓰는 기본 타입이 몇 가지 더 있다.
정수는 `int`, 참/거짓 두 값만 갖는 불리언은 `bool`, 소수점 있는 숫자는 `float64`다.
5장에서 볼 SRS 상태 구조체가 `float64`와 `int` 필드로 이뤄져 있어 곧 실물로 만난다.

변수를 만드는 방법은 두 가지다.
`var 이름 타입` 형태의 정식 선언과, 선언과 대입을 한 번에 하는 단축 선언 `:=`다.
cmd/server/main.go에서 이미 본 한 줄이 단축 선언의 전형이다.

```go
	port := os.Getenv("PORT")
```

`os.Getenv`는 환경 변수의 값을 문자열로 돌려주는 표준 라이브러리 함수다.
`:=` 왼쪽의 `port`는 이 자리에서 새로 선언되고, 오른쪽 값이 문자열이므로 타입도 `string`으로 자동 결정된다.
타입을 일일이 적지 않아도 컴파일러가 오른쪽을 보고 알아내는 것을 타입 추론(Type Inference)이라고 한다.

같은 선언을 정식으로 풀어 쓰면 `var port string = os.Getenv("PORT")`가 된다.
...
Go에서는 초기화하지 않은 변수에 쓰레기값이 아니라 타입별로 정해진 기본값이 들어가는데, 이를 제로값(Zero Value)이라고 한다.
숫자는 `0`, 문자열은 빈 문자열 `""`, 불리언은 `false`, 슬라이스와 포인터는 "없음"을 뜻하는 `nil`이다.
초기화를 잊어도 변수가 늘 예측 가능한 상태에서 출발한다는 뜻이다.
제로값은 5장에서 "제로값이 곧 유효한 초기 상태가 되도록 구조체를 설계한다"는 원칙으로 다시 등장하니 기억해 두자.

## 함수와 다중 반환값

config.go의 나머지 절반은 `Load` 함수 하나다.
환경 변수를 읽어 `Config`를 채우고, 필수 값이 빠져 있으면 에러를 돌려주는 함수다.
...
함수 선언은 `func 이름(매개변수) 반환값` 순서로 읽는다.
`Load`는 매개변수가 없고, 반환값이 `(*Config, error)` 두 개다.
`*Config`의 `*`는 `Config`가 놓인 곳을 가리키는 포인터(Pointer) 표시인데, 포인터는 5장에서 제대로 다룬다.
지금은 반환값의 짝에 집중하자.
성공하면 채워진 설정과 `nil`(에러 없음)을, 실패하면 `nil`(설정 없음)과 에러를 돌려준다.

여기에 Go 에러 처리의 뼈대가 되는 관례가 있다.
Go에는 다른 언어의 예외(Exception)처럼 오류가 별도 통로로 튀어 오르는 장치가 없다.
...
다른 언어의 예외(exception)처럼 오류가 몰래 튀어 오르는 대신 반환값으로 눈앞에 놓이므로, 확인을 빠뜨리기 어렵다.
:::

호출하는 쪽의 모양은 앞서 본 cmd/server/main.go에 있다.
Gin 엔진을 조립하는 `app.Engine()`을 부르고 결과를 받는 두 줄이다.
...
반환값 두 개를 `engine, err :=`로 한 번에 받고, 바로 다음 줄에서 `err != nil`인지 검사한다.
...
Go에는 `public`, `private` 같은 키워드가 없다.
식별자(함수, 타입, 필드, 상수의 이름)가 대문자로 시작하면 패키지 밖으로 공개(exported)되고, 소문자로 시작하면 그 패키지 안에서만 쓸 수 있다.
`Load`와 `Config`가 대문자인 덕분에 다른 패키지에서 `config.Load()`처럼 불러 쓸 수 있고, main.go가 부른 `app.Engine()`의 `Engine`도 마찬가지다.
...
main.go의 마지막에 있던 `if err := engine.Run(":" + port); err != nil`도 같은 형태로, 함수 호출과 에러 검사를 한 줄로 묶은 관용구다.
...
이 코드가 놓인 HTTP 핸들러의 전체 구조는 7장의 주제이므로, 여기서는 문법만 확인하고 넘어가자.
...
이 파일의 인코딩이 정말 겹치지 않는 슬러그를 만드는지 확인하는 테스트는 6장에서 읽는다.
...
다음 장에서는 이 기초 위에서 Echo Flip의 도메인 코드를 읽는다.
