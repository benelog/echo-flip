...
여기서 다진 기초는 다음 3장에서 간격 반복(Spaced Repetition) 알고리즘을 구현한 internal/srs 패키지를 비롯한 도메인 코드를 본격적으로 읽기 위한 준비 운동이다.
...
구조체는 3장의 주인공이니 여기서는 "이름표가 붙은 칸이 다섯 개 있는 상자" 정도로 이해하고 넘어가자.
...
제로값은 3장에서 "제로값이 곧 유효한 초기 상태가 되도록 구조체를 설계한다"는 원칙으로 다시 등장하니 기억해 두자.
...
`*Config`의 `*`는 `Config`가 놓인 곳을 가리키는 포인터(Pointer) 표시인데, 포인터는 3장에서 제대로 다룬다.
지금은 반환값의 짝에 집중하자.
...
이 코드가 놓인 HTTP 핸들러의 전체 구조는 5장의 주제이므로, 여기서는 문법만 확인하고 넘어가자.
...
이 파일의 인코딩이 정말 겹치지 않는 슬러그를 만드는지 확인하는 테스트는 4장에서 읽는다.
