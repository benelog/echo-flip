...
다른 파일이나 남의 패키지 코드를 가져다 쓰겠다고 파일 맨 위에 적는 선언이 임포트(import)이고, 그때 적는 주소가 임포트 경로다.
...
표준 라이브러리는 언어를 설치하면 따라오는 기본 부품 모음이라, 남의 코드를 받아 오지 않아도 HTTP 서버·JSON·시간 처리가 이미 들어 있다.
...
다른 언어에서는 문제가 생기면 오류가 호출한 쪽으로 저절로 튀어 올라 어딘가에서 잡히기를 기다리는데, 이 장치를 예외(Exception)라고 한다.
...
코드에는 '남들이 갖다 써도 되는 것'과 '내부 사정이라 감춰 둘 것'의 구분이 필요하다.
많은 언어가 `public`, `private` 같은 키워드로 이 구분을 적어 두는데, Go에는 그런 키워드가 없고 이름의 첫 글자가 그 일을 대신한다.
