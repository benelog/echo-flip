...
이 장의 중심 예제는 환경 변수를 읽어 서버 설정을 만드는 internal/config/config.go로, 60줄짜리 파일 하나에 이 장의 문법 요소가 거의 다 들어 있다.
...
`require` 블록에는 JWT 검증(keyfunc, golang-jwt), 웹 프레임워크(gin), DB 드라이버(pgx, sqlite) 등 직접 의존성 여덟 개가 전부인데, 이 적은 수 자체가 표준 라이브러리의 두터움을 보여 준다.
...
가운데의 `if` 분기는 설정에 따라 SQLite 파일로 도는 로컬 모드와 PostgreSQL 모드를 가르는데, 그 사정은 7장과 14장에서 다루므로 지금은 문법의 재료로만 쓰겠다.
...
파일의 앞부분으로, 설정값 여덟 개를 한 묶음으로 담을 그릇을 정의한다.
...
구조체는 5장의 주인공이니 여기서는 "이름표가 붙은 칸이 여덟 개 있는 상자" 정도로 이해하고 넘어가자.
...
환경 변수를 읽어 `Config`를 채우고, `DATABASE_URL`의 유무로 PostgreSQL 모드와 SQLite 로컬 모드를 가른 뒤, 필수 값이 빠져 있으면 에러를 돌려주는 함수다.
...
각 모드에서 무엇이 달라지는지는 7장과 14장의 몫이다.
