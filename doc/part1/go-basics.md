...
여기서 다진 기초는 다음 7장에서 간격 반복(Spaced Repetition) 알고리즘을 구현한 internal/srs 패키지를 비롯한 도메인 코드를 본격적으로 읽기 위한 준비 운동이다.
...
가운데의 `if` 분기는 설정에 따라 SQLite 파일로 도는 로컬 모드와 PostgreSQL 모드를 가르는데, 그 사정은 9장과 13장에서 다루므로 지금은 문법의 재료로만 쓰겠다.
