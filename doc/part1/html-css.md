...
::: info [용어 풀이] 화면 낭독기(screen reader)
화면에 있는 것을 소리로 읽어 주는 프로그램이다.
눈으로 화면을 보기 어려운 사용자가 웹을 쓰는 통로이며, 글자만 읽는 것이 아니라 "여기는 이동 메뉴", "버튼, 정답 보기"처럼 각 부분이 무엇인지도 함께 알려 준다.
그러려면 문서가 그 정보를 담고 있어야 하므로, `lang` 속성이나 다음 절의 시맨틱 태그처럼 겉모습에 드러나지 않는 표시가 낭독기에게는 중요한 단서가 된다.
...
CSS는 이 이름표로 꾸밀 대상을 찾고, `<label for="reveal">`처럼 요소끼리 연결할 때나 12장에서 htmx가 화면의 어느 자리를 갈아 끼울지 정할 때도 id로 그 자리를 짚는다.
:::
...
::: info [용어 풀이] 요소의 가족 관계(부모·자식·형제·조상·자손)
HTML은 태그가 태그를 품는 나무 모양의 구조라(12장에서 볼 DOM이 이 나무다), 요소들이 놓인 자리를 가계도의 낱말로 부른다.
...
CSS 선택자는 이 낱말들로 꾸밀 요소를 짚고(`.bottomnav a`는 자손, `input:checked + span`은 형제), 12장의 htmx도 `closest li`처럼 같은 관계로 갈아 끼울 자리를 가리킨다.
:::
...
자바스크립트로 화면 전환을 직접 구현하는 SPA 방식과의 비교는 1장에서 다뤘고, 이 방식의 빈틈(전체 새로고침이 어색한 장면)을 메우는 htmx는 12장에서 만난다.
...
Flashcard의 스타일은 internal/web/static/app.css 한 파일, 800줄 남짓이 전부다.
...
Flashcard의 화면은 될 수 있으면 상태를 HTML 요소에 들려 두고 CSS가 그것을 읽어 겉모습만 바꾸며, 12장의 학습 화면은 진행 상태를 폼에 실어 서버와 왕복시킨다.
:::
