...
5장에서 읽은 SRS 알고리즘이나 스마트 덱 규칙이 정말 설계대로 동작하는지, 눈으로 코드를 읽는 것만으로는 확신하기 어렵다.
확인 작업을 사람 손에 맡기는 대신 검사 코드로 적어 두고 기계가 반복 실행하게 하는 것이 테스트다.

이 장에서는 먼저 Go가 테스트를 찾아 실행하는 규칙과 명령 사용법을 익힌다.
이어서 5장에서 읽은 코드들의 실제 테스트를 읽으며 테이블 주도 테스트(Table-driven Test) 패턴과 전수 검사, 경계값 검사를 확인한다.
마지막으로 gofmt, go vet 같은 품질 검사 도구를 살펴보고, gofmt가 강제하는 세로 정렬과 그 대가를 따로 짚는다.
이 테스트와 도구들은 사람을 위한 안전망인 동시에, 11장과 12장에서 다룰 AI 에이전트가 자기 작업을 스스로 검증하는 신호이기도 하다.
...
4장에서 본 공개 규칙(대문자만 공개)이 같은 패키지의 테스트에는 장벽이 되지 않는다.
...
5장에서 `Grade`가 현재 시각을 인자로 받도록 설계한 것을 봤는데, 그 덕분에 테스트는 고정 시각을 주입해 언제 실행해도 같은 결과를 얻는다.
...
4장 상수 절에서 본 internal/store/deckslug.go는 덱 번호(seq)를 네 글자 슬러그로 바꾸는 `encodeDeckSlug`와 그 역변환 `decodeDeckSlug`를 담고 있다.
...
충돌 검사에는 `seen` 맵이 쓰였는데, 5장에서 본 comma-ok 관용구(`prev, dup := seen[slug]`)로 "이 슬러그가 전에 나왔는지"를 확인하고, 나왔다면 어느 번호와 부딪혔는지까지 메시지에 담는다.
...
테스트가 AI 협업에서 갖는 의미는 11장에서 본격적으로 다루고, 테스트 실행을 강제하는 자동화는 12장에서 다룬다.
...
이 프로젝트에서는 AI 에이전트가 Go 파일을 수정할 때마다 훅(Hook)으로 gofmt와 go vet이 자동 실행되도록 구성했는데, 그 구조와 설계 의도는 12장에서 자세히 다룬다.
...
한 줄을 더했을 뿐인데 `git blame`은 나머지 네 줄의 책임 커밋도 이 커밋으로 옮긴다.
...
gofmt는 컬럼 너비를 화면에 보이는 폭이 아니라 룬 개수로 센다.
...
다섯째, gofmt는 선언부의 세로 정렬까지 강제한다.
정렬 스타일을 고를 여지가 없어 논쟁이 사라지는 대신 diff가 넓어지는 대가가 따르고, 빈 줄로 정렬 그룹을 끊어 완화한다.
