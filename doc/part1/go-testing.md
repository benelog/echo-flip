...
테스트가 AI 협업에서 갖는 의미는 10장에서 본격적으로 다루고, 테스트 실행을 강제하는 자동화는 11장에서 다룬다.
...
이 프로젝트에서는 AI 에이전트가 Go 파일을 수정할 때마다 훅(Hook)으로 gofmt와 go vet이 자동 실행되도록 구성했는데, 그 구조와 설계 의도는 11장에서 자세히 다룬다.
