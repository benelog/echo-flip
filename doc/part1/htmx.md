...
::: info [용어 풀이] hidden 필드(hidden field)
...
`data-tts`나 `data-copy`는 HTML 표준이 정해 둔 이름이 아니다.
`data-` 뒤에 오는 이름은 개발자가 마음대로 지어 쓰라고 열어 둔 자유 칸이고, 브라우저는 그 값을 화면에 그리지 않고 자바스크립트가 꺼내 쓸 데이터로만 보관한다.
`data-tts`는 이 앱이 "이 버튼이 눌리면 이 글을 읽어 달라"는 뜻으로 지어낸 이름이며, `data-copy`도 마찬가지다.
