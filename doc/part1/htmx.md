# 10장 htmx: 자바스크립트 없이 만드는 동적 화면

8장에서 화면의 구조와 모양을 만드는 HTML·CSS를, 9장에서 서버가 데이터를 채워 HTML을 완성해 내려보내는 html/template을 살펴봤다.
...
9장에서 만든 서버 렌더링 구조가 그대로 연장되므로, 화면을 그리는 로직은 여전히 Go 한 곳에만 있다.
...
htmx.min.js는 50KB 남짓한 파일 하나로, 외부에서 받아 오는 대신 저장소에 그대로 담아(vendoring) 9장에서 본 embed로 Go 바이너리에 함께 넣었다.
빌드 도구도, 패키지 설치도 없다.
...
다음은 internal/web/templates/pages/deck.html의 카드 목록으로, 삭제 버튼에 htmx 속성 세 개가 붙어 있다.
...
9장에서 본 render는 layout 템플릿부터 실행해 `<html>`로 시작하는 페이지 전체를 만들었다.
...
partials가 templates/partials/ 아래 파일들로 만들어지던 것은 9장의 템플릿 파싱 코드에서 봤다.
...
흐름을 따라가 보자.
stateFromForm이 폼 값에서 studyState를 다시 조립하고, 큐의 첫 카드를 꺼내 채점 결과를 데이터베이스에 기록한다(이것이 서버에 남는 유일한 것으로, 3장에서 설계한 리뷰 기록이다).
...
셋째, 요청마다 상태 전체가 왕복한다.
카드 ID 목록이 수백 개라면 요청 하나가 몇 KB씩 무거워진다.
...
버튼 하나가 폼 전체를 보내고, 응답 하나가 화면의 네 곳으로 갈라져 내려앉는다.
사용자가 이미 뜻을 적어 두었다면 dictionaryLookup이 그 값을 받은 그대로 되돌려 주므로, 교체가 일어나도 입력이 지워지지 않는다.

사전 조회를 브라우저가 아니라 서버가 하는 것도 짚어 둘 만하다.
외부 API 호출과 응답 가공이 Go 코드에 있으므로 브라우저 자바스크립트가 필요 없고, 응답을 두 줄 요약으로 다듬는 로직(mapEntries)은 6장에서 본 방식대로 Go 테스트로 검증할 수 있다.
...
카드 뒤집기조차 8장에서 본 대로 숨긴 체크박스와 CSS만으로 동작한다.
...
이 장으로 8장에서 시작한 화면 이야기 세 편이 마무리됐다.
8장에서 문서(HTML)와 스타일(CSS)을, 9장에서 서버가 데이터를 채우는 html/template을, 이번 장에서 새로고침 없는 동적 화면을 만드는 htmx를 살펴봤다.
...
다음 11장에서는 시선을 코드 자체에서 코드를 만드는 과정으로 돌려, 이 앱을 실제로 함께 만든 도구인 Claude Code를 살펴보겠다.
