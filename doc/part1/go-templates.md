# 9장 html/template으로 만드는 화면

7장에서 만든 API는 JSON 데이터를 돌려줄 뿐이다.
덱 목록을 요청하면 중괄호와 따옴표로 묶인 텍스트가 올 뿐, 사람이 볼 화면은 아니다.
8장에서는 그 화면의 재료인 HTML과 CSS를 익혔다.
이제 남은 질문은 하나다.
...
Echo Flip은 의존성이 늘지 않고 자동 이스케이프가 기본인 표준 라이브러리 html/template를 골랐고, 컴파일 검사가 없다는 약점은 이 장 마지막 절에서 테스트로 메운다.

Echo Flip의 템플릿은 `internal/web/templates/` 아래에 있다.
페이지 12장(`pages/*.html`), 공통 뼈대 1장(`layout.html`), 여러 페이지가 나눠 쓰는 조각 3개(`partials/*.html`)가 전부다.
...
Echo Flip의 템플릿은 까다로운 대목마다 이 주석으로 의도를 적어 둔다.
방금 본 학습 조각도 `{{/* 학습 진행 상태를 hidden 필드로 실어 나른다. ... */}}`라는 주석으로 시작한다.

## 레이아웃과 조각: 뼈대는 한 번만 쓴다

홈, 덱 목록, 통계, 설정 어느 페이지를 열어도 상단 바와 하단 내비게이션은 같다.
페이지 12장마다 이 뼈대를 복사해 두면 뼈대를 고칠 때마다 12곳을 고쳐야 하므로, 뼈대는 `layout.html` 한 장에만 쓰고 페이지는 저마다의 본문만 쓴다.
...
여기서 문제가 하나 생긴다.
템플릿 집합 하나 안에서 같은 이름은 하나만 존재할 수 있는데, 페이지 12장이 전부 `content`라는 이름을 쓴다.
모든 파일을 한 집합에 파싱하면 마지막에 파싱된 `content`가 앞의 것들을 덮어써 버린다.
그래서 Echo Flip은 레이아웃과 조각을 담은 공통 집합(base)을 만들고, 페이지 수만큼 복제(Clone)한 뒤 복제본마다 페이지 파일 하나씩을 얹어 페이지마다 독립된 집합을 만든다.
...
조각을 단독으로 그리는 `renderPartial`도 있는데, 페이지 일부만 갈아 끼우는 htmx 응답용이므로 10장에서 다룬다.
...
이 중 `icon`은 특별하다.
화면 곳곳의 아이콘(집, 불꽃, 휴지통 등)을 이미지 파일이나 아이콘 폰트 없이 인라인 SVG로 그리는 함수다.
...
왜 필요한지부터 보자.
Echo Flip에서 카드의 원문과 뜻은 사용자가 입력하는 값이다.
...
Echo Flip은 값을 원문 그대로 넘기고 인코딩은 템플릿의 URL 문맥 이스케이프 한 곳에 맡긴 뒤, 그 결정을 구조체 주석으로 남겨 두었다.
...
Echo Flip은 배포 산출물이 "실행 파일 하나"로 끝나는 단순함을 택해 embed를 골랐다.
...
7장의 API 핸들러가 "요청을 읽고, 스토어를 다녀와서, JSON으로 응답"이었다면, 페이지 핸들러는 마지막 단계만 "템플릿 렌더링"으로 바뀐다.
...
7장의 JSON 바인딩과 하는 일은 같고, 데이터가 JSON 본문 대신 폼 인코딩으로 온다는 점만 다르다.

주목할 부분은 성공했을 때 `render`가 아니라 `c.Redirect`를 부른다는 것이다.
POST 응답으로 화면을 직접 그리면 함정이 생긴다.
사용자가 그 화면에서 새로 고침을 누르면 브라우저는 "양식을 다시 제출하시겠습니까?"라고 묻고, 수락하는 순간 같은 덱이 하나 더 만들어진다.
그래서 POST는 처리만 하고 곧장 GET 주소로 이동시키는 것이 오래된 관례이고, 이를 Post/Redirect/Get, 줄여서 PRG 패턴이라고 부른다.
이동 후의 화면은 평범한 GET 결과라 몇 번을 새로 고쳐도 조회만 반복되고, 상태 코드도 "다음 요청은 GET으로 하라"는 뜻의 303 See Other(`http.StatusSeeOther`)를 쓴다.
...
Echo Flip은 수명이 짧은 쿠키로 이 틈을 잇는다.
POST 핸들러가 메시지를 쿠키에 실어 보내면 다음 페이지를 그릴 때 꺼내 쓰고 바로 지우는데, 한 번 보이고 사라지는 성질 때문에 플래시 메시지(flash message)라고 부른다.
...
`internal/web/templates_test.go`는 페이지 12장이 모두 파싱됐는지 확인하고, 가장 복잡한 학습 화면 조각을 국면(phase)마다 실제로 렌더링해 본다.
...
그 정도만으로도 필드 이름 오타나 조각 이름 변경 같은 흔한 실수는 배포 전에 걸러지고, 6장에서 만든 `go test ./...` 한 번에 같이 돈다.
...
레이아웃 발췌가 읽어 들이던 `htmx.min.js`가 그 답의 손잡이다.
