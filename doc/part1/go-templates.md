...
파싱은 텍스트를 읽어 그 구조를 프로그램이 다룰 수 있는 형태로 해석해 두는 일이다.
템플릿 파싱은 `{{if}}`나 `{{range}}` 같은 표기가 어디서 시작해 어디서 끝나는지를 미리 해석해, 이제 데이터만 채우면 되는 상태로 준비해 두는 것이다.
서버가 켜질 때 한 번만 파싱해 두는 이유는 요청이 올 때마다 파일을 다시 읽고 해석하는 일을 피하기 위해서다.
...
SVG(Scalable Vector Graphics)는 그림을 점과 선의 좌표로 적어 두는 형식이라, 사진처럼 별도의 그림 파일로 두지 않고 HTML 안에 글자처럼 직접 써 넣을 수 있다(이렇게 문서 안에 직접 써 넣는 것을 인라인이라고 한다).
그래서 아이콘 파일을 따로 내려받게 하거나 아이콘 폰트(아이콘 그림들을 글꼴 하나로 묶어 배포하는 방식)를 싣지 않아도 되고, 색과 크기는 CSS로 바꿀 수 있다.
...
브라우저가 이 쪽지를 다음 요청부터 자동으로 함께 보내 주므로, 리다이렉트 뒤의 GET 요청에도 직전 POST가 남긴 메시지가 딸려 온다.
