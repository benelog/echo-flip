...
2장부터 12장까지 Echo Flip의 코드를 층층이 읽어 왔다.
데이터가 저장되는 테이블에서 출발해 Go API와 서버가 렌더링하는 화면을 지나, 그 코드를 만들어 내는 에이전트의 작업 환경까지 올라왔다.
이번 장은 1부의 피날레다.
...
이것으로 실행은 끝이다.
9장에서 봤듯 이 Go 서버 하나가 HTML 화면과 JSON API를 모두 서빙하므로, 프로세스를 하나 더 띄울 일이 없다.
...
맞음과 틀림을 고를 때마다 3장의 테이블에 복습 기록이 쌓이고, 5장의 SRS 함수가 다음 복습 시각을 계산하고, 9장의 템플릿과 10장의 htmx가 그 결과를 화면에 그린다.
...
데이터베이스 백업이라는 말의 무게에 비하면 싱거울 정도인데, 운영 데이터베이스의 백업은 20장에서 이보다 훨씬 심각한 주제로 다시 만난다.
...
`golang.go`는 Go 공식 확장으로 뒤에서 볼 포맷·검사·디버깅이 모두 여기에 얹히고, `anthropic.claude-code`는 11장의 Claude Code를 에디터 안에서 띄우는 확장이다.

목록이 두 개뿐인 것은 의도한 결과다.
이 저장소의 코드는 Go 하나로 수렴하고(화면도 서버가 렌더링하는 템플릿과 순수 CSS, 몇십 줄의 JS라 별도 도구 사슬이 없다), 포맷은 설정 옵션이 없는 gofmt가 독점하므로 포매터나 린터 확장을 따로 들일 이유가 없었다.
...
6장의 `go build`·`go vet`·`go test`는 모두 터미널에서 치면 되는 명령이다.
...
6장에서 본 테이블 주도 테스트가 실패할 때, 실패한 케이스의 입력값을 중단점에서 직접 들여다보는 용도다.
...
저장할 때 gofmt를 돌리고 `go vet`을 확인하는 일은, 12장에서 본 훅이 에이전트에게 하는 일과 정확히 같다.
차이는 대상이다.
...
11장에서 본 CLI를 터미널에서 그대로 써도 되고 `anthropic.claude-code` 확장으로 에디터 안에서 띄워도 되는데, 확장을 쓰면 에이전트의 변경 사항이 에디터의 diff 뷰로 열려 좌우를 나란히 놓고 볼 수 있다.
...
에디터의 저장 시 포맷과 12장의 훅은 같은 규칙을 사람 쪽 입구와 에이전트 쪽 입구에 각각 걸어 둔 구성이다.

다섯째, 로컬 모드는 환경 변수가 없을 때의 기본값으로 설계됐다.
`SQLITE_PATH`와 `PORT` 같은 손잡이는 지금도 쓸 수 있고, `DATABASE_URL`이라는 가장 큰 손잡이는 18장에서 잡는다.

이것으로 1부가 끝났다.
데이터의 모양을 정하고(2·3장), 그것을 다루는 서버를 읽고(4~7장), 화면을 붙이고(8~10장), 코드를 만들어 내는 에이전트와 사람의 환경을 갖췄으며(11~13장), 그 전부가 방금 독자의 컴퓨터에서 돌았다.
