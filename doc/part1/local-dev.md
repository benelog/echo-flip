...
```
2026/07/11 06:15:20 echo-flip api listening on :8080 (local mode, sqlite: local-db/echo-flip.db)
```

`local mode`라는 표시를 눈여겨보자.
아무 설정도 주지 않았으므로 서버가 로컬 모드로 떴고, 데이터는 `local-db/echo-flip.db`라는 SQLite 파일에 저장된다는 뜻이다.
이것으로 실행은 끝이다.
...
데이터베이스 드라이버는 SQLite(파일은 `local-db/echo-flip.db`), 인증 모드는 로컬이다.
...
앱을 조금 만져 봤다면 저장소에 `local-db/` 폴더가 생기고 그 안에 `echo-flip.db`가 만들어졌을 것이다(쓰는 도중에는 `-wal`·`-shm`이 붙은 작업 파일 두 개도 함께 보인다).
이 파일이 데이터베이스 전부다.
...
이 코드는 담길 폴더와 SQLite 파일을 없으면 만들어 가며 열고, 소스에 함께 임베드된 스키마를 곧바로 적용한다.
...
서버를 잠시 멈춘 뒤(Ctrl+C) 저장소 루트에서 `sqlite3 local-db/echo-flip.db`를 실행한다.
...
sqlite3로 한 번 열었다 닫으면 작업 파일의 내용이 본 파일로 합쳐지므로 `local-db/echo-flip.db` 하나만 복사해도 된다.
...
여기서 쓰는 VS Code(Visual Studio Code)는 마이크로소프트가 무료로 배포하는 코드 편집기이고, 그 편집기에 기능을 얹어 주는 부가 프로그램을 확장(extension)이라 부른다.
...
::: info [용어 풀이] 디버거(Debugger)와 중단점(Breakpoint)
...
diff는 파일의 어디가 어떻게 바뀌었는지 보여 주는 비교 결과인데, 코드의 뜻은 그대로이고 공백과 줄바꿈만 달라진 변경이 거기 섞이면 정작 무엇을 고쳤는지가 잘 보이지 않는다.
...
셋째, 내 데이터는 `local-db/echo-flip.db` 파일 하나에 있다.
