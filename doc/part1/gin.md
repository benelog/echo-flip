...
화면은 9장에서 다룰 서버 렌더링 템플릿이 담당하므로, 이 장의 주인공인 `/api/*` 경로는 사람이 아니라 프로그램을 위한 창구로 남는다.
...
처음부터 계층을 겹겹이 쌓기보다, API 응답과 내부 모델이 갈라지거나 두 번째 구현처럼 필요가 증명될 때 분리하는 편이 작은 프로젝트에는 맞다.
