# 3장 Gin으로 만드는 HTTP API

2장에서는 Go 언어의 문법과 Echo Flip 백엔드의 패키지 구조를 살펴봤다.
...
이런 Vercel 함수의 제약과 배포 구성은 9장에서 자세히 다룬다.
...
::: info 용어 · CORS(Cross-Origin Resource Sharing)
...
서명 검증 키는 JWKS(JSON Web Key Set) URL에서 가져오는데, 그 상세 원리와 Supabase 인증 체계는 10장에서 다룬다.
