...
6장과 7장에서는 Go 언어의 문법과 Echo Flip 백엔드의 패키지 구조를 살펴봤다.
...
요청과 응답은 각각 부가 정보를 적는 헤더(header)와 실제 내용이 담기는 본문(body)으로 나뉘는데, 봉투 겉면에 적는 발신인이나 취급 주의 표시가 헤더이고 그 안에 든 편지지가 본문이다.
...
`New`가 구체적인 DB 타입이 아니라 `handlers.Store` 인터페이스를 받는 덕에 어떤 저장소 구현이든 꽂을 수 있는데, 이 설계의 효용은 이 장 끝의 계층 분리 절에서 다시 만난다.

::: info [용어 풀이] 인터페이스(Interface)
...
둘째, `Engine`은 `sync.Once`로 엔진을 프로세스당 한 번만 만든다.
프로세스(process)는 실행 중인 프로그램 하나를 말하는데, 서버가 켜져서 꺼질 때까지가 프로세스 하나의 수명이고 그동안 메모리에 만들어 둔 것은 계속 재사용할 수 있다.
...
패닉(panic)은 Go 프로그램이 더 진행할 수 없는 오류를 만나 그 자리에서 실행을 중단하는 것으로, 일상어의 "당황"과 달리 프로그램이 실제로 멈춰 선다는 뜻이다.
복구 미들웨어는 그 중단을 요청 하나의 실패(500 응답)로 가둬 서버 프로세스 전체가 멈추지 않게 하므로, 이것만 남겼다.
...
첫째, 진입점은 둘이어도 조립은 `New` 하나이므로, 로컬과 프로덕션이 같은 라우팅·핸들러·미들웨어 순서를 지나 "로컬에서는 됐는데 배포하니 다르다"류의 문제가 줄어든다.
...
요청은 주소에도 값을 실어 온다.
주소 `/api/decks/ab3f`에서 덱마다 달라지는 `ab3f` 자리가 경로 파라미터(path parameter)이고, 주소 뒤에 `?dueBefore=2026-07-12T09:00:00Z`처럼 물음표를 붙여 덧대는 부가 조건이 쿼리 파라미터(query parameter)다.
...
Echo Flip에서는 화면과 API가 같은 Go 서버에서 나가므로(로컬은 8080 포트 하나, 배포는 같은 Vercel 도메인) 앱 자신의 화면이 API를 부를 때는 교차 출처가 아예 발생하지 않는다.
...
허용 출처는 하드코딩하지 않고 환경 변수 `ALLOWED_ORIGINS`에서 읽는다(internal/config/config.go).
이 변수를 비워 두면(기본 상태) 미들웨어 자체가 등록되지 않는다.
같은 출처끼리는 검사가 없으니 아무것도 필요 없고, 이 미들웨어가 일하는 것은 별도 오리진에 올린 웹페이지가 이 API를 직접 호출하는 예외적 구성뿐이다.
그런 클라이언트를 붙일 일이 생기면 `ALLOWED_ORIGINS`에 그 오리진을 적어 주는 것으로 문이 열린다.
...
`Middleware`는 설정값을 받아 `gin.HandlerFunc`를 반환하는 팩토리 함수(factory function)다.
설정값을 받아 함수를 만들어 돌려주는 함수를 팩토리 함수라 하고, 그렇게 만들어진 함수가 자기를 만든 자리의 값들(여기서는 `jwksURL`과 `secret`)을 기억한 채 살아 있는 것을 클로저(closure)라 한다.
...
그 덕에 스토어를 모의 객체(mock)로 바꾸는 핸들러 단위 테스트의 길도 함께 열렸다.
모의 객체는 진짜 데이터베이스 대신 미리 정해 둔 답만 돌려주는 가짜 스토어라, DB 없이 핸들러의 동작만 따로 시험해 볼 수 있게 해 준다.
