...
넷째, `build()` 안에서 실패할 수 있는 일(설정 누락, 잘못된 드라이버, DB 연결 실패)은 모두 에러로 반환한다.
...
이런 Vercel 함수의 제약과 배포 구성은 17장에서 자세히 다룬다.
...
서명 검증 키는 JWKS(JSON Web Key Set) URL에서 가져오는데, 그 상세 원리와 Supabase 인증 체계는 18장에서 다룬다.
...
검증 없는 인증이 왜 로컬에서만 허용되는지, 어떤 안전장치가 운영 유출을 막는지는 18장에서 다룬다.
...
미들웨어 체인은 Recovery → CORS → JWT 인증(로컬 모드에서는 고정 사용자) → 프로필 보장 순으로 흐르고, 인증 미들웨어가 실어 준 사용자 ID를 모든 핸들러가 `auth.UserID(c)`로 꺼내 쓴다.
