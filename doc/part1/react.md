...
TypeScript 문법은 6장과 7장에서 다뤘다고 전제한다.
컴포넌트들을 하나의 앱으로 조립하는 Context와 서버 상태 관리, Next.js의 라우팅과 정적 export는 다음 9장에서 다룬다.
...
`decks?.`의 물음표는 6장에서 본 옵셔널 체이닝으로, 서버에서 덱 목록이 아직 도착하지 않아 `undefined`일 때를 건너뛴다(이 데이터를 받아오는 쪽은 9장에서 다룬다).
...
`createDeck.mutate(...)`는 서버에 덱 생성을 요청하는 부분인데, 서버와의 대화는 9장의 주제이므로 지금은 "제출하면 서버에 요청이 간다" 정도로만 읽고 넘어가자.
...
9장에서 볼 `AuthProvider`가 인증 이벤트 구독을 이 패턴으로 관리하고, `src/hooks/useTts.ts`도 `voiceschanged` 이벤트 리스너를 같은 방식으로 정리한다.
...
리듀서는 React 없이도 호출할 수 있으므로, 7장에서 본 vitest로 상태 전이를 단위 테스트하기 쉽다.
...
다음 9장에서는 이 부품들이 하나의 앱으로 조립되는 층을 다룬다.
