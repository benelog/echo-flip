...
TypeScript 문법은 6장과 7장에서 다뤘다고 전제하고, 배포 세부 구성은 12장으로 미룬다.
...
이름은 같지만 11장에 나오는 Claude Code의 훅과는 다른 것이다.
:::
...
왜 이 한 줄이 필요한지는 인증 흐름 전체를 다루는 13장에서 자세히 살펴본다.
...
앞서 말했듯 리렌더링 때는 컴포넌트 함수가 통째로 다시 실행되므로, 그냥 만들면 화면이 갱신될 때마다 캐시 창고가 새로 지어져 모아 둔 데이터가 사라진다.
...
rewrite를 포함한 배포 구성 전체는 12장에서 자세히 다룬다.
...
다음 9장에서는 이 화면들이 읽고 쓰는 데이터의 뼈대, PostgreSQL 데이터베이스 설계로 내려간다.
