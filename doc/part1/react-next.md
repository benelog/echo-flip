...
React와 Next.js, 정적 export라는 배포 형태를 선택한 이유는 1장에서 정리했고, 배포 세부 구성은 16장으로 미룬다.
...
왜 이 한 줄이 필요한지는 인증 흐름 전체를 다루는 17장에서 자세히 살펴본다.
...
나머지 절반인 서비스 워커(`public/sw.js`)와 각 필드가 실제 설치 경험에 주는 영향은 19장에서 따로 다룬다.
...
rewrite를 포함한 배포 구성 전체는 16장에서 자세히 다룬다.
...
여기까지가 1부다.
데이터베이스라는 바닥에서 출발해 Go와 Gin으로 API를 올리고, TypeScript와 React로 사용자가 만지는 화면까지 한 층씩 쌓아 올렸다.

다음 장부터는 2부로 넘어간다.
12장의 Claude Code를 시작으로, 지금까지 읽은 이 코드를 실제로 만들어 낸 AI 에이전트와 그 앱이 돌아가는 인프라를 살펴본다.
