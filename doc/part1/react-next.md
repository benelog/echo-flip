# 9장 React 실전과 Next.js: 전역 상태와 정적 export

8장이 컴포넌트라는 부품을 만드는 장이었다면, 이 장은 그 부품들이 Echo Flip이라는 하나의 앱으로 조립되는 층을 다룬다.
...
React와 Next.js, 정적 export라는 배포 형태를 선택한 이유는 1장에서 정리했고, 배포 세부 구성은 14장으로 미룬다.
...
물론 진짜 보안 경계는 Go API의 토큰 검증(5장)이고, 이 가드는 사용자 경험을 위한 장치일 뿐이다.
...
8장의 목록 렌더링 예제에서 `decks?.map`의 물음표가 감당하던 "아직 도착하지 않은 데이터"가 바로 이 `data`다.
...
8장의 덱 생성 폼에서 "제출하면 서버에 요청이 간다"라고만 해 둔 `createDeck.mutate`의 실체가 이것이다.
...
8장에서 말했듯 리렌더링 때는 컴포넌트 함수가 통째로 다시 실행되므로, 그냥 만들면 화면이 갱신될 때마다 캐시 창고가 새로 지어져 모아 둔 데이터가 사라진다.
...
rewrite를 포함한 배포 구성 전체는 14장에서 자세히 다룬다.

### 'use client' 지시어

8장과 이 장에서 발췌한 컴포넌트 파일은 대부분 첫 줄이 `"use client"`다.
...
다음 10장에서는 이 화면들이 읽고 쓰는 데이터의 뼈대로 내려간다.
관계형 데이터베이스가 데이터를 담는 방식과 SQL, 인덱스라는 기초를 다지고, 11장에서 Echo Flip의 실제 스키마 설계를 해부한다.
