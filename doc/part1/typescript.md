...
다음 7장은 실행 중에 벌어지는 일, 즉 네트워크 응답을 기다리는 비동기 처리와 바깥에서 들어오는 데이터의 런타임 검증, 그리고 코드 품질을 지키는 도구를 다룬다.
React와 Next.js 자체는 8장과 9장의 몫이므로, 이 장은 언어로서의 TypeScript에 집중한다.
...
`useState`는 화면의 상태(state)를 담아 두는 React 함수인데, 자세한 동작은 8장에서 다루므로 여기서는 "괄호 안의 초기값으로 시작하는 값 하나와, 그 값을 바꾸는 함수 하나를 만들어 준다" 정도로만 이해하면 된다.
...
다음 7장에서는 타입이 지켜 주지 못하는 실행 시점의 세계로 들어간다.
