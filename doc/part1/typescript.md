# 3장 TypeScript — 타입으로 지키는 프런트엔드

Echo Flip의 백엔드가 Go라면 프런트엔드의 언어는 TypeScript다.
이번 장에서는 왜 브라우저에서 동작하는 코드에 굳이 타입 시스템을 얹었는지, 그 선택의 트레이드오프부터 짚어 보겠다.
이어서 `src/lib/` 아래의 실제 코드를 재료로 인터페이스, 유니온 타입, 제네릭 같은 언어 기능을 살펴보고, 컴파일 타임 타입이 보장하지 못하는 런타임 검증의 간극과 이를 메우는 방법을 다룬다.
마지막으로 strict 모드의 `tsconfig.json`과 vitest 테스트 등 타입과 코드 품질을 지키는 도구를 정리한다.
React와 Next.js 자체는 4장에서 자세히 다루므로, 이 장은 언어로서의 TypeScript에 집중한다.
...
백엔드 언어는 선택지가 넓다.
1장에서 본 것처럼 Go, Node.js, JVM 계열 등 여러 후보를 놓고 서버 환경의 제약에 맞춰 고를 수 있었다.
그러나 프런트엔드는 사정이 다르다.
브라우저가 직접 실행하는 언어는 사실상 자바스크립트(JavaScript)뿐이다.
웹어셈블리(WebAssembly)라는 선택지가 있지만 DOM 조작과 UI 프레임워크 생태계를 생각하면 일반적인 웹 앱의 주 언어가 되기는 아직 어렵다.
...
타입 검사가 사람의 개입 없이 도는 피드백 루프의 첫 관문이 되는 셈이다.
이런 검증을 훅(hook)으로 자동화하는 방법은 7장에서 자세히 다룬다.
...
Echo Flip이 쓰는 라이브러리를 보면 Next.js는 프로젝트 생성 시 TypeScript가 기본값이고, TanStack Query는 타입 추론을 전제로 API를 설계했으며, zod는 아예 타입 시스템과의 연동이 존재 이유다.
...
앞서 말했듯 이 명령은 에이전트가 만든 코드의 1차 관문으로 쓰기 좋고, 사람의 리뷰는 이 관문을 통과한 코드에 집중하면 된다.
...
1장에서 본 Go의 테이블 주도 테스트와 형태는 다르지만, 입력과 기대 출력을 나열한다는 본질은 같다.
...
첫째, 브라우저의 언어가 사실상 자바스크립트뿐이라는 제약 아래에서, TypeScript는 동적 타입의 한계(런타임에야 드러나는 오류)를 컴파일 시점 검증으로 보완하는 선택이다.
타입 검사는 AI 에이전트가 생성한 코드를 사람 개입 없이 즉시 검증하는 신호로도 쓰이며, React 생태계의 사실상 표준이다.
다만 수명이 짧은 프로토타입이나 빌드 없는 스크립트에는 순수 자바스크립트(+JSDoc)가 더 나을 수 있다.
...
다음 4장에서는 이 타입들 위에서 화면을 만드는 React와 Next.js를 다룬다.
