# 7장 TypeScript 실전: 비동기 처리와 런타임 검증

6장에서 본 타입 시스템은 실행 전에 잘못을 걸러 주는 안전망이었다.
이번 장은 실행 중의 세계를 다룬다.
...
오류 처리 방식도 눈여겨보자.
3장에서 본 Go가 `(값, err)` 다중 반환으로 오류를 값으로 다뤘다면, TypeScript는 예외를 던진다.
...
6장에서 본 fetch 래퍼 `request<T>`가 어떻게 끝나는지 다시 보자.
...
`ApiEntry`의 모든 필드가 6장에서 본 옵셔널 프로퍼티인데, 외부 API가 무엇을 빠뜨릴지 모른다는 전제를 타입에 반영한 것이다.
...
4장에서 본 Go의 테이블 주도 테스트와 형태는 다르지만, 입력과 기대 출력을 나열한다는 본질은 같다.
