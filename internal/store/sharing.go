...
func (s *Store) GetSharedDeckCards(ctx context.Context, slug string) ([]SharedCard, error) {
	rows, err := s.pool.Query(ctx,
		`select c.front_text, c.back_text, c.card_type, c.tags, c.phonetic, c.example, c.notes
		 from cards c
	...
	for rows.Next() {
		var c SharedCard
		if err := rows.Scan(&c.FrontText, &c.BackText, &c.CardType, &c.Tags,
...
func (s *Store) ImportSharedDeck(ctx context.Context, viewerID uuid.UUID, slug string) (Deck, error) {
	...
	if _, err := tx.Exec(ctx,
		`with copied as (
		   insert into cards (user_id, deck_id, front_text, back_text, card_type,
		                      tags, phonetic, example, notes)
		   select $1, $2, front_text, back_text, card_type, tags, phonetic, example, notes
