...
func TestDeckSlugRoundTrip(t *testing.T) {
	for _, seq := range []int64{1, 2, 35, 36, 100, 1295} {
...
// Exhaustively round-trips every addressable seq, guarding against an
// arithmetic slip in the permutation or its inverse and confirming slugs are
// always 2 chars and never collide.
func TestDeckSlugBijective(t *testing.T) {
	seen := make(map[string]int64)
	for seq := int64(1); seq < slugSpace; seq++ {
		slug := encodeDeckSlug(seq)
		if len(slug) != slugLen {
			t.Fatalf("encodeDeckSlug(%d) = %q, want %d chars", seq, slug, slugLen)
		}
...
func TestDeckSlugNotSequential(t *testing.T) {
	if a, b := encodeDeckSlug(1), encodeDeckSlug(2); a == "01" || b == "02" {
...
func TestDeckSlugCaseInsensitive(t *testing.T) {
	for _, seq := range []int64{1, 42, 1295} {
		slug := encodeDeckSlug(seq)
...
func TestDecodeDeckSlugInvalid(t *testing.T) {
	...
	for _, s := range []string{"", "a", "abc", "a!", "한", "00"} {
